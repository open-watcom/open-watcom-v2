...
unsigned char               EchoStack[STACKSIZE];
TID                         EchoId;
unsigned char               WaiterStack[STACKSIZE];
TID                         WaiterId;
RESULTCODES                 ChildId;
HFILE                       StdinRdHdl;
HFILE                       StdinWrHdl;
...
static void SetOurCwd( char * buff )
/***********************************
...
*/
    ...
    for( ;; ) {
        ...
        *++p = '\0';
        ...
        for( ;; ) {
            *++p = ReadCh();
            if( strncmp( (p - endlen) + 1, END, endlen ) == 0 )
                break;
            if( *p == '\n' ) {
                break;
            }
