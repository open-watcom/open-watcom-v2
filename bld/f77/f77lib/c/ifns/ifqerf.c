...
//
// IFQERF       : error function for extended PRECISION argument
//
...
//==========================
    ...
    if( arg < __qerfdat.pt477 ) {
        // abs(arg) < .477, evaluate approximation for erf()
        ...
        // .477 <= abs(arg) <= 4.0 evaluate approximation for erf()
        xsq = arg*arg;
        ...
        res = res*exp(-xsq);
        res = 1.0-res;
    } else if( arg < __qerfdat.xlarge ) {
        // 4.0 < abs(arg), evaluate minimax approximation for erf()
        xsq = arg*arg;
        xi = 1.0/xsq;
