/****************************************************************************
*
*                            Open Watcom Project
*
* Copyright (c) 2002-2026 The Open Watcom Contributors. All Rights Reserved.
*    Portions Copyright (c) 1983-2002 Sybase, Inc. All Rights Reserved.
*
*  ========================================================================
*
*    This file contains Original Code and/or Modifications of Original
*    Code as defined in and that are subject to the Sybase Open Watcom
*    Public License version 1.0 (the 'License'). You may not use this file
*    except in compliance with the License. BY USING THIS FILE YOU AGREE TO
*    ALL TERMS AND CONDITIONS OF THE LICENSE. A copy of the License is
*    provided with the Original Code and Modifications, and is also
*    available at www.sybase.com/developer/opensource.
*
*    The Original Code and all software distributed under the License are
*    distributed on an 'AS IS' basis, WITHOUT WARRANTY OF ANY KIND, EITHER
*    EXPRESS OR IMPLIED, AND SYBASE AND ALL CONTRIBUTORS HEREBY DISCLAIM
*    ALL SUCH WARRANTIES, INCLUDING WITHOUT LIMITATION, ANY WARRANTIES OF
*    MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE, QUIET ENJOYMENT OR
*    NON-INFRINGEMENT. Please see the License for the specific language
*    governing rights and limitations under the License.
*
*  ========================================================================
*
* Description:  F-Code processor for subroutine calls.
*
****************************************************************************/


#include "ftnstd.h"
#include "global.h"
#include "i64.h"
#include "wf77aux.h"
#include "wf77cg.h"
#include "tmpdefs.h"
#include "cpopt.h"
#include "fcgbls.h"
#include "iflookup.h"
#include "prmcodes.h"
#include "fmemmgr.h"
#include "emitobj.h"
#include "fctypes.h"
#include "argchk.h"
#include "fccmplx.h"
#include "fcflow.h"
#include "fcstring.h"
#include "fcsubscr.h"
#include "fcsyms.h"
#include "fctemp.h"
#include "forcstat.h"
#include "rstmgr.h"
#include "fcjmptab.h"
#include "fccall.h"
#include "fcdata.h"
#include "fcfield.h"
#include "fcstack.h"
#include "wf77info.h"
#include "cgswitch.h"
#include "cgprotos.h"
#include "feprotos.h"


extern  void            FiniTmps(void);

extern  back_handle     TraceEntry;
extern  segment_id      CurrCodeSegId;

/* Forward declarations */
static  void    GenTraceback( void );
static  void    GenReturnValue( sym_id sym );
static  void    GenCommonReturnValue( void );
static  void    DefineEntries( void );
static  void    PassCommonArgs( call_handle call, entry_pt *ep_called );



cg_type SPType( sym_id sym )
//============================
// Return subprogram cg type.
{
    if( (sym->u.ns.flags & SY_SUBPROG_TYPE) == SY_PROGRAM )
        return( TY_INTEGER );
    if( (sym->u.ns.flags & SY_SUBPROG_TYPE) == SY_SUBROUTINE )
        return( TY_INTEGER );
    // must be a function
    if( sym->u.ns.u1.s.typ == FT_CHAR )
        return( TY_INTEGER );
    return( F77ToCGType( sym ) );
}


static cg_type      CommonEntryType( void )
//=========================================
// Generate return value for common entry point.
{
    return( TY_INTEGER );
}


void    FCPrologue( void )
//====================
// Start a subprogram.
{
    sym_id              sym;
    entry_pt            *ep;
    signed_64           ep_count;
    sel_handle          sel;
    label_handle        main_entry_label;
    unsigned_16         sp_type;

    --NumSubProgs;
    sym = GetPtr();
    sp_type = (sym->u.ns.flags & SY_SUBPROG_TYPE);
    SubCodeSeg();
    BESetSeg( SEG_LDATA );
    DGSeek( LDSegOffset );
    if( sp_type == SY_BLOCK_DATA ) {
        if( (sym->u.ns.flags & SY_UNNAMED) == 0 ) {
            DGLabel( FEBack( sym ) );
        }
    } else {
        if( CommonEntry == NULL ) {
            CGProcDecl( sym, SPType( sym ) );
        } else {
            CGProcDecl( CommonEntry, CommonEntryType() );
        }
        if( CGOpts & CGOPT_DB_LOCALS ) {
            DBModSym( sym, TY_DEFAULT );
        }
    }
    GenLocalSyms();
    if( CGOpts & CGOPT_DB_LOCALS ) {
        if( sp_type != SY_BLOCK_DATA ) {
            GenLocalDbgInfo();
        }
    }
    if( sp_type != SY_BLOCK_DATA ) {
        GenTraceback();
    }
    if( CommonEntry != NULL ) {
        sel = CGSelInit();
        Set64Val1p( ep_count );
        for( ep = Entries->link; ep != NULL; ep = ep->link ) {
            CGSelCase( sel, GetCgLabel( ep->id->u.ns.si.sp.u.entry ), ep_count );
            U64Inc( ep_count );
        }
        main_entry_label = BENewLabel();
        CGSelOther( sel, main_entry_label );
        CGSelect( sel, CGUnary( O_POINTS, CGFEName( EPValue, TY_INTEGER ), TY_INTEGER ) );
        CGControl( O_LABEL, NULL, main_entry_label );
        BEFiniLabel( main_entry_label );
    }
    BESetSeg( SEG_LDATA );
    LDSegOffset = DGTell();
}


static  void    GenTraceback( void )
//==================================
{
    call_handle         call;

    if( Options & OPT_TRACE ) {
        call = InitCall( RT_SET_MODULE );
        CGAddParm( call, CGBackName( TraceEntry, TY_POINTER ), TY_POINTER );
        CGDone( CGCall( call ) );
    }
}


void    FCEpilogue( void )
//========================
// End a subprogram.
{
    sym_id      sym;
    unsigned_16 sp_type;

    sym = GetPtr();
    sp_type = (sym->u.ns.flags & SY_SUBPROG_TYPE);
    if( sp_type != SY_BLOCK_DATA ) {
        GenTraceback();
    }
    if( StNumbers.wild_goto ) {
        DoneLabel( StNumbers.branches );
    }
    FiniLabels( false );
    FiniTmps();
    if( (sym->u.ns.flags & SY_SUBPROG_TYPE) == SY_PROGRAM ) {
        CGReturn( NULL, TY_INTEGER );
    } else if( sp_type != SY_BLOCK_DATA ) {
        if( CommonEntry == NULL ) {
            GenReturnValue( sym );
        } else {
            GenCommonReturnValue();
            DefineEntries();
        }
    }
    BEFlushSeg( CurrCodeSegId );
    FreeLocalBacks( true );
    FreeGlobalBacks();
}


bool    ChkForAltRets( entry_pt *ep )
//===================================
// Check to see if the specified entry point has an alternate return.
{
    parameter   *args;

    for( args = ep->parms; args != NULL; args = args->link ) {
        if( args->flags & ARG_STMTNO ) {
            return( true );
        }
    }
    return( false );
}


bool    EntryWithAltRets( void )
//==============================
// Check to see if there's at least one entry with an alternate return.
{
    entry_pt    *ep;

    for( ep = Entries; ep != NULL; ep = ep->link ) {
        if( ChkForAltRets( ep ) ) {
            return( true );
        }
    }
    return( false );
}


static  void    GenReturnValue( sym_id sym )
//==========================================
// Generate return value.
{
    cg_type     cgtyp;

    cgtyp = SPType( sym );
    if( (sym->u.ns.flags & SY_SUBPROG_TYPE) == SY_SUBROUTINE ) {
        if( !EntryWithAltRets() ) {
            CGReturn( NULL, TY_INTEGER );
            return;
        }
    } else { // must be a function
        if( sym->u.ns.u1.s.typ == FT_CHAR ) {
            CGReturn( NULL, TY_INTEGER );
            return;
        }
    }
    CGReturn( CGUnary( O_POINTS, CGFEName( ReturnValue, cgtyp ), cgtyp ), cgtyp );
}


static  void    GenCommonReturnValue( void )
//==========================================
// Generate return value for common entry point.
{
    if( EntryWithAltRets() ) {
        CGReturn( CGUnary( O_POINTS, CGFEName( ReturnValue, TY_INTEGER ),
                           TY_INTEGER ), TY_INTEGER );
    } else {
        CGReturn( NULL, TY_INTEGER );
    }
}


static  void    DefineEntries( void )
//===================================
// Define entry points into subprogram.
{
    entry_pt    *ep;
    int         ep_count;
    call_handle call;
    cg_type     sp_cgtyp;
    cg_type     common_cgtyp;
    cg_name     val;

    common_cgtyp = CommonEntryType();
    ep_count = 0;
    for( ep = Entries; ep != NULL; ep = ep->link ) {
        FreeLocalBacks( false );
        sp_cgtyp = SPType( ep->id );
        CGProcDecl( ep->id, sp_cgtyp );
        if( CGOpts & CGOPT_DB_LOCALS ) {
            DBModSym( ep->id, TY_DEFAULT );
        }
        DefineEntryPoint( ep );
        call = CGInitCall( CGFEName( CommonEntry, common_cgtyp ), common_cgtyp, NULL );
        PassCommonArgs( call, ep );
        if( (ep->id->u.ns.flags & SY_SUBPROG_TYPE) != SY_SUBROUTINE ) {
            val = CGFEName( ReturnValue, sp_cgtyp );
            if( ep->id->u.ns.u1.s.typ == FT_CHAR ) {
                val = CGUnary( O_POINTS, val, TY_POINTER );
                CGAddParm( call, val, TY_POINTER );
                if( (Options & OPT_DESCRIPTOR) == 0 ) {
                    val = CGFEName( FindArgShadow( ReturnValue ), TY_INTEGER );
                    val = CGUnary( O_POINTS, val, TY_INTEGER );
                    CGAddParm( call, val, TY_INTEGER );
                }
            } else {
                CGAddParm( call, val, TY_POINTER );
            }
        }
        CGAddParm( call, CGInteger( ep_count, TY_INTEGER ), TY_INTEGER );
        if( (ep->id->u.ns.flags & SY_SUBPROG_TYPE) == SY_SUBROUTINE ) {
            if( EntryWithAltRets() ) {
                val = CGUnary( O_POINTS, CGCall( call ), sp_cgtyp );
                CGReturn( val, sp_cgtyp );
            } else {
                CGDone( CGCall( call ) );
                CGReturn( NULL, sp_cgtyp );
            }
        } else {
            CGDone( CGCall( call ) );
            if( ep->id->u.ns.u1.s.typ == FT_CHAR ) {
                CGReturn( NULL, TY_INTEGER );
            } else {
                CGReturn( CGUnary( O_POINTS, CGFEName( ReturnValue, sp_cgtyp ), sp_cgtyp ), sp_cgtyp );
            }
        }
        ep_count++;
    }
}


static  bool    NeedShadowArg( pass_by *arg_aux )
//===============================================
{
    if( Options & OPT_DESCRIPTOR )
        return( false );
    if( arg_aux == NULL )
        return( true );
    if( arg_aux->info & (PASS_BY_VALUE | PASS_BY_DATA) )
        return( false );
    return( true );
}


static  void    PassCommonArgs( call_handle call, entry_pt *ep_called )
//=====================================================================
// Pass arguments to common entry point.
{
    entry_pt    *ep;
    parameter   *arg;
    cg_name     cg;
    cg_type     arg_cgtyp;
    aux_info    *info;
    pass_by     *arg_aux;

    for( ep = Entries; ep != NULL; ep = ep->link ) {
        info = InfoLookup( ep->id );
        arg_aux = info->arg_info;
        for( arg = ep->parms; arg != NULL; arg = arg->link ) {
            if( (arg->flags & (ARG_DUPLICATE | ARG_STMTNO)) == 0 ) {
                if( (arg->id->u.ns.flags & SY_CLASS) == SY_SUBPROGRAM ) {
                    arg_cgtyp = TY_CODE_PTR;
                } else if( arg->id->u.ns.flags & SY_SUBSCRIPTED ) {
                    arg_cgtyp = TY_POINTER;
                } else if( arg->id->u.ns.u1.s.typ == FT_CHAR ) {
                    arg_cgtyp = TY_POINTER;
                } else if( ( arg_aux != NULL )
                  && (arg_aux->info & PASS_BY_VALUE) ) {
                    arg_cgtyp = F77ToCGType( arg->id );
                } else {
                    arg_cgtyp = TY_POINTER;
                }
                if( InArgList( ep_called, arg->id ) ) {
                    cg = CGUnary( O_POINTS, CGFEName( arg->id, arg_cgtyp ), arg_cgtyp );
                    if( arg->id->u.ns.u1.s.typ == FT_CHAR ) {
                        CGAddParm( call, cg, PromoteCGToBaseType( arg_cgtyp ) );
                        if( NeedShadowArg( arg_aux ) ) {
                            cg = CGFEName( FindArgShadow( arg->id ), TY_INTEGER );
                            cg = CGUnary( O_POINTS, cg, TY_INTEGER );
                            CGAddParm( call, cg, TY_INTEGER );
                        }
                    } else {
                        CGAddParm( call, cg, PromoteCGToBaseType( arg_cgtyp ) );
                    }
                } else {
                    CGAddParm( call, CGInteger( 0, arg_cgtyp ), PromoteCGToBaseType( arg_cgtyp ) );
                    if( arg->id->u.ns.u1.s.typ == FT_CHAR ) {
                        if( NeedShadowArg( arg_aux ) ) {
                            CGAddParm( call, CGInteger( 0, TY_INTEGER ), TY_INTEGER );
                        }
                    }
                }
            }
            if( ( arg_aux != NULL )
              && ( arg_aux->link != NULL ) ) {
                arg_aux = arg_aux->link;
            }
        }
    }
}


static  cg_name     ArgAddr( PTYPE ptyp )
//=======================================
// Get address of actual argument.
{
    cg_type     cgtyp;
    cg_name     arg;

    arg = XPop();
    cgtyp = GetCGType( ptyp );
    if( !IsCGPointer( CGType( arg ) ) ) {
        if( ( ptyp == FPT_CPLX_8 )
          || ( ptyp == FPT_CPLX_16 )
          || ( ptyp == FPT_CPLX_32 ) ) {
            arg = CmplxAddr( arg, XPop() );
        } else {
            arg = TmpPtr( MkTmp( arg, cgtyp ), cgtyp );
        }
    }
    return( arg );
}


static  cg_name     ArgValue( PTYPE ptyp, cg_type *new_cgtyp )
//============================================================
// Get value of actual argument.
{
    cg_name     arg;
    cg_type     cgtyp;

    arg = XPop();
    cgtyp = CGType( arg );
    if( IsCGPointer( cgtyp ) ) {
        if( !IsCGCodePointer( cgtyp ) ) {
            cgtyp = GetCGType( ptyp );
            arg = CGUnary( O_POINTS, arg, cgtyp );
        }
    } else {
        cgtyp = GetCGType( ptyp );
    }
    *new_cgtyp = cgtyp;
    return( arg );
}


static  cg_type ArgPtrType( cg_name arg )
//=======================================
// Get argument pointer type.
{
    cg_type     cgtyp;

    cgtyp = CGType( arg );
    if( cgtyp >= TY_USER_DEFINED )
        return( TY_POINTER );
    if( BETypeLength( cgtyp ) < BETypeLength( TY_POINTER ) )
        return( TY_POINTER );
    return( cgtyp );
}


#if _CPU == 386

static  cg_name MkSCB16( cg_name src )
//====================================
{
    cg_name     src_2;
    cg_name     dst;
    cg_name     dst_2;

    dst = TmpPtr( AllocTmp( TY_CHAR16 ), TY_CHAR16 );
    CloneCGName( src, &src, &src_2 );
    CloneCGName( dst, &dst, &dst_2 );
    CGTrash( CGAssign( SCBLenAddr( dst_2 ), SCBLength( src_2 ), TY_UINT_2 ) );
    // Assumption: first field in SCB is pointer to data
    return( CGLVAssign( SCBPtrAddr( dst ),
             CGUnary( O_PTR_TO_FOREIGN, SCBPointer( src ), TY_GLOBAL_POINTER ),
             TY_GLOBAL_POINTER ) );
}

#endif


void    FCCall( void )
//====================
// Process CALL F-Code.
{
    sym_id      sp;
    sym_id      scb;
    args_num    argc;
    uint        idx;
    call_handle call;
    cg_type     sp_cgtyp;
    aux_info    *info;
    cg_name     rtn;
    unsigned_16 arg_info;
    PTYPE       ptyp;
    PCODE       pcode;
    pass_by     *arg_aux;
    cg_name     arg;
    cg_type     cgtyp;
    cg_type     new_cgtyp;
    cg_cmplx    z;
    bool        chk_foreign;
    bool        pass_scb;
    bool        pass_len;
    cg_name     *arg_vec;
#if _INTEL_CPU
    bool        arg_proc_far16;
#endif

    scb = NULL;
    sp = GetPtr();
    sp_cgtyp = SPType( sp );
    info = InfoLookup( sp );
    arg_aux = info->arg_info;
    if( sp->u.ns.flags & SY_SUB_PARM ) {
        rtn = CGUnary( O_POINTS, CGFEName( sp, TY_CODE_PTR ), TY_CODE_PTR );
    } else {
        rtn = CGFEName( sp, sp_cgtyp );
    }
    argc = GetU16();
    if( argc == 0 ) {
        arg_vec = NULL;
    } else {
        // We need argc + 1 because we use NULL at the end
        arg_vec = FMemAlloc( ( argc + 1 ) * sizeof( cg_name ) );
    }
    call = CGInitCall( rtn, sp_cgtyp, sp );
    if( sp->u.ns.flags & SY_INTRINSIC ) {
        if( IFVarArgs( sp->u.ns.si.fi.index ) ) {
            CGAddParm( call, CGInteger( argc, TY_INTEGER ), TY_INTEGER );
        }
    } else if( (sp->u.ns.flags & SY_SUBPROG_TYPE) == SY_FUNCTION ) {
        if( (Options & OPT_DESCRIPTOR) == 0
          && (sp->u.ns.u1.s.typ == FT_CHAR) ) {
            scb = GetPtr();
            arg = SCBPointer( CGFEName( scb, TY_CHAR ) );
#if _CPU == 386
            if( info->cclass_target & FECALL_X86_FAR16_CALL ) {
                arg = CGUnary( O_PTR_TO_FOREIGN, arg, TY_POINTER );
            }
#endif
            CGAddParm( call, arg, TY_POINTER );
            CGAddParm( call, SCBLength( CGFEName( scb, TY_CHAR ) ), TY_INTEGER );
        }
    }
    idx = 0;
    for( ; argc != 0; --argc ) {
        arg_info = GetU16();
        ptyp = _GetArgInfoPtyp( arg_info );
        pcode = _GetArgInfoPcode( arg_info );
#if _INTEL_CPU
        arg_proc_far16 = ( (pcode & PC_PROC_FAR16) != 0 );
        pcode &= ~PC_PROC_FAR16;
#endif
        chk_foreign = true;
        if( pcode == PC_PROCEDURE
          || pcode == PC_FN_OR_SUB ) {
            arg = XPop();
            cgtyp = TY_CODE_PTR;
#if _INTEL_CPU
            if( (info->cclass_target & FECALL_X86_FAR16_CALL)
              && arg_proc_far16 ) {
                chk_foreign = false;
            } else if( arg_aux != NULL ) {
                if( arg_aux->info & ARG_FAR ) {
                    cgtyp = TY_LONG_CODE_PTR;
                    arg = CGUnary( O_CONVERT, arg, TY_LONG_CODE_PTR );
                    chk_foreign = false;
                }
            }
#endif
        } else if( ptyp == FPT_CHAR ) {
            // character data (including character arrays)
            if( Options & OPT_DESCRIPTOR ) {
                pass_scb = true;
                pass_len = false;
            } else {
                pass_scb = false;
                pass_len = true;
            }
            if( arg_aux != NULL ) {
                if( arg_aux->info & (PASS_BY_VALUE | PASS_BY_DATA) ) {
                    pass_scb = false;
                    pass_len = false;
                } else if( arg_aux->info & PASS_BY_DESCRIPTOR ) {
                    pass_scb = true;
                    pass_len = false;
                } else if( arg_aux->info & PASS_BY_NODESCRIPTOR ) {
                    pass_scb = false;
                    pass_len = true;
                }
            }
            arg = XPop();
            cgtyp = TY_POINTER;
            if( pass_len ) {
                CloneCGName( arg, &arg, &arg_vec[idx] );
                ++idx;
            }
#if _CPU == 386
            if( pass_scb
              && (info->cclass_target & FECALL_X86_FAR16_CALL) ) {
                arg = MkSCB16( arg );
            }
#endif
            if( !pass_scb ) {
                arg = SCBPointer( arg );
            }
            if( arg_aux != NULL ) {
                if( arg_aux->info & ARG_FAR ) {
                    cgtyp = TY_LONG_POINTER;
                    arg = CGUnary( O_CONVERT, arg, TY_LONG_POINTER );
                }
            }
        } else if( pcode == PC_ARRAY_NAME ) {
            arg = XPop();
            cgtyp = ArgPtrType( arg );
            if( arg_aux != NULL ) {
                if( (arg_aux->info & PASS_BY_REFERENCE)
                  && (arg_aux->info & ARG_FAR) ) {
                    cgtyp = TY_LONG_POINTER;
                    arg = CGUnary( O_CONVERT, arg, TY_LONG_POINTER );
                    chk_foreign = false;
                }
            }
        } else if( (arg_aux != NULL)
          && (arg_aux->info & PASS_BY_VALUE) ) {
            chk_foreign = false;
            if( ptyp == FPT_CPLX_8 ) {
                XPopCmplx( &z, TY_COMPLEX );
                CGAddParm( call, z.realpart, TY_SINGLE );
                arg = z.imagpart;
                cgtyp = TY_SINGLE;
            } else if( ptyp == FPT_CPLX_16 ) {
                XPopCmplx( &z, TY_DCOMPLEX );
                CGAddParm( call, z.realpart, TY_DOUBLE );
                arg = z.imagpart;
                cgtyp = TY_DOUBLE;
            } else if( ptyp == FPT_CPLX_32 ) {
                XPopCmplx( &z, TY_XCOMPLEX );
                CGAddParm( call, z.realpart, TY_LONGDOUBLE );
                arg = z.imagpart;
                cgtyp = TY_LONGDOUBLE;
            } else if( ptyp == FPT_STRUCT ) {
                arg = ArgValue( ptyp, &cgtyp );
            } else {
                arg = ArgValue( ptyp, &cgtyp );
                new_cgtyp = cgtyp;
                if( IntType( ptyp ) ) {
                    if( arg_aux->info & ARG_SIZE_1 ) {
                        new_cgtyp = TY_INT_1;
                    } else if( arg_aux->info & ARG_SIZE_2 ) {
                        new_cgtyp = TY_INT_2;
                    } else if( arg_aux->info & ARG_SIZE_4 ) {
                        new_cgtyp = TY_INT_4;
                    }
                } else if( ( ptyp == FPT_LOG_1 )
                  || ( ptyp == FPT_LOG_4 ) ) {
                    if( arg_aux->info & ARG_SIZE_1 ) {
                        new_cgtyp = TY_UINT_1;
                    } else if( arg_aux->info & ARG_SIZE_2 ) {
                        new_cgtyp = TY_UINT_2;
                    } else if( arg_aux->info & ARG_SIZE_4 ) {
                        new_cgtyp = TY_UINT_4;
                    }
                } else { // must be FPT_REAL_4 or FPT_REAL_8 or FPT_REAL_16
                    if( arg_aux->info & ARG_SIZE_4 ) {
                        new_cgtyp = TY_SINGLE;
                    } else if( arg_aux->info & ARG_SIZE_8 ) {
                        new_cgtyp = TY_DOUBLE;
                    } else if( arg_aux->info & ARG_SIZE_16 ) {
                        new_cgtyp = TY_LONGDOUBLE;
                    }
                }
                if( cgtyp != new_cgtyp ) {
                    arg = CGUnary( O_CONVERT, arg, new_cgtyp );
                    cgtyp = new_cgtyp;
                } else if( (sp->u.ns.flags & SY_INTRINSIC)
                  && ( IFArgType( sp->u.ns.si.fi.index ) == FT_INTEGER ) ) {
                    // The following code is only useful when the intrinsic
                    // promotion switch is activated.  We don't need to check
                    // for the switch because if the switch was not set then
                    // an error would be issued before we ever reach this code.
                    if( ( cgtyp == TY_INT_1 )
                      || ( cgtyp == TY_INT_2 ) ) {
                        cgtyp = TY_INT_4;
                        arg = CGUnary( O_CONVERT, arg, TY_INT_4 );
                    }
                }
                cgtyp = PromoteCGToBaseType( cgtyp );
            }
        } else {
            arg = ArgAddr( ptyp );
            cgtyp = ArgPtrType( arg );
            if( arg_aux != NULL ) {
                if( (arg_aux->info & PASS_BY_REFERENCE)
                  && (arg_aux->info & ARG_FAR) ) {
                    cgtyp = TY_LONG_POINTER;
                    arg = CGUnary( O_CONVERT, arg, TY_LONG_POINTER );
                    chk_foreign = false;
                }
            }
        }
#if _INTEL_CPU
        if( (info->cclass_target & FECALL_X86_FAR16_CALL)
          && chk_foreign ) {
            arg = CGUnary( O_PTR_TO_FOREIGN, arg, cgtyp );
        }
#endif
        CGAddParm( call, arg, cgtyp );
        if( (arg_aux != NULL)
          && (arg_aux->link != NULL) ) {
            arg_aux = arg_aux->link;
        }
    }
    if( arg_vec != NULL ) {
        arg_vec[idx] = NULL;
        for( idx = 0; arg_vec[idx] != NULL; ++idx ) {
            CGAddParm( call, SCBLength( arg_vec[idx] ), TY_INTEGER );
        }
        FMemFree( arg_vec );
    }
    if( (sp->u.ns.flags & SY_SUBPROG_TYPE) == SY_SUBROUTINE ) {
        XPush( CGUnary( O_POINTS, CGCall( call ), sp_cgtyp ) );
    } else {
        if( sp->u.ns.u1.s.typ == FT_CHAR ) {
            if( (Options & OPT_DESCRIPTOR)
              || (sp->u.ns.flags & SY_INTRINSIC) ) {
                scb = GetPtr();
                arg = CGFEName( scb, TY_CHAR );
#if _CPU == 386
                if( info->cclass_target & FECALL_X86_FAR16_CALL ) {
                    arg = MkSCB16( arg );
                    arg = CGUnary( O_PTR_TO_FOREIGN, arg, TY_GLOBAL_POINTER );
                }
#endif
                CGAddParm( call, arg, TY_POINTER );
            }
            XPush( CGBinary( O_COMMA, CGCall( call ), CGFEName( scb, TY_CHAR ), TY_LOCAL_POINTER ) );
        } else {
            if( (sp_cgtyp == TY_COMPLEX)
              || (sp_cgtyp == TY_DCOMPLEX)
              || (sp_cgtyp == TY_XCOMPLEX) ) {
                SplitCmplx( CGCall( call ), sp_cgtyp );
            } else if( sp_cgtyp >= TY_USER_DEFINED ) { // structure
                XPush( CGCall( call ) );
            } else {
                XPush( CGUnary( O_POINTS, CGCall( call ), sp_cgtyp ) );
            }
        }
    }
}

#if 0
void        FCEvalArg( void )
//===========================
// Force evaluation of an argument.
{
    XPush( CGEval( XPop() ) );
}
#endif

cg_name VarAltSCB( sym_id sym )
//=============================
// This function is called for character variables that are dummy arguments
// and SCBRequired() is true.
{
    /*
    for dummy arguments, the SCB is always allocated on the stack
    so we have no problems with recursion
    if( Options & OPT_AUTOMATIC ) {
    */
        return( CGTempName( sym->u.ns.si.va.vi.alt_scb, TY_CHAR ) );
    /*
    } else {
        return( CGBackName( sym->u.ns.si.va.vi.alt_scb, TY_CHAR ) );
    }
    */
}


cg_name SubAltSCB( sym_id sym )
//=============================
// This function is called for function return values when
// the /nodescriptor option is specified
{
    /*
    for function return values, the SCB is always allocated on the stack
    so we have no problems with recursion
    if( Options & OPT_AUTOMATIC ) {
    */
        return( CGTempName( sym->u.ns.si.sp.alt_scb, TY_CHAR ) );
    /*
    } else {
        return( CGBackName( sym->u.ns.si.sp.alt_scb, TY_CHAR ) );
    }
    */
}


void    FCDArgInit( void )
//========================
// Initialize dummy arguments.
{
    sym_id      sym;
    cg_name     arg;
    cg_name     dst;

    sym = GetPtr();
    if( (Options & OPT_DESCRIPTOR) == 0 ) {
        if( (sym->u.ns.flags & SY_SUBPROG_TYPE) == SY_FUNCTION ) {
            if( sym->u.ns.u1.s.typ == FT_CHAR ) {
                arg = CGUnary( O_POINTS, CGFEName( ReturnValue, TY_POINTER ), TY_POINTER );
                if( Options & OPT_DESCRIPTOR ) {
                    arg = SCBLength( arg );
                } else {
                    if( CommonEntry != NULL ) {
                        sym = CommonEntry;
                    }
                    dst = SCBPtrAddr( SubAltSCB( sym ) );
                    CGDone( CGAssign( dst, arg, TY_POINTER ) );
                    arg = CGUnary( O_POINTS,
                             CGFEName( FindArgShadow( ReturnValue ), TY_INTEGER ), TY_INTEGER );
                }
                dst = SCBLenAddr( SubAltSCB( sym ) );
                CGDone( CGAssign( dst, arg, TY_INTEGER ) );
            }
        }
    }
    for( ; (sym = GetPtr()) != NULL; ) {
        if( (sym->u.ns.flags & SY_CLASS) != SY_VARIABLE )
            continue;
        if( sym->u.ns.u1.s.typ != FT_CHAR )
            continue;
        if( sym->u.ns.flags & SY_SUBSCRIPTED )
            continue;
        if( !SCBRequired( sym ) )
            continue;
        // character variable
        arg = CGUnary( O_POINTS, CGFEName( sym, TY_POINTER ), TY_POINTER );
        dst = SCBPtrAddr( VarAltSCB( sym ) );
        if( (sym->u.ns.flags & SY_VALUE_PARM) == 0
          && (Options & OPT_DESCRIPTOR) ) {
            arg = SCBPointer( arg );
        }
        CGDone( CGAssign( dst, arg, TY_POINTER ) );
        if( sym->u.ns.xt.size == 0 ) {
            dst = SCBLenAddr( VarAltSCB( sym ) );
            if( sym->u.ns.flags & SY_VALUE_PARM ) {
                arg = CGInteger( 0, TY_INTEGER );
            } else if( Options & OPT_DESCRIPTOR ) {
                arg = CGFEName( sym, TY_POINTER );
                arg = CGUnary( O_POINTS, arg, TY_POINTER );
                arg = SCBLength( arg );
            } else {
                arg = CGFEName( FindArgShadow( sym ), TY_INTEGER );
                arg = CGUnary( O_POINTS, arg, TY_INTEGER );
            }
            CGDone( CGAssign( dst, arg, TY_INTEGER ) );
        }
    }
}


void    FCPassFieldCharArray( void )
//==================================
// Pass character array, array element, or substring array element to
// subprogram.  Character array is a field within a structure.
{
    sym_id      fd;
    sym_id      scb;

    fd = GetPtr();
    scb = GetPtr();
    MakeSCB( scb, FieldArrayEltSize( fd ) );
}


void    FCPassCharArray( void )
//=============================
// Pass character array, array element, or substring array element to
// subprogram.
{
    sym_id      arr;
    sym_id      scb;

    arr = GetPtr();
    scb = GetPtr();
    MakeSCB( scb, ArrayEltSize( arr ) );
}


void    FCAltReturn( void )
//=========================
// Process alternate return.
{
    sel_handle      sel;
    int             num_alts;
    signed_64       alt_ret;
    obj_ptr         curr_obj;
    label_handle    other;

    curr_obj = FCodeTell( 0 );
    sel = CGSelInit();
    Set64Val1p( alt_ret );
    num_alts = GetU16();
    while( num_alts-- > 0 ) {
        CGSelCase( sel, GetStmtCgLabel( GetPtr() ), alt_ret );
        U64Inc( alt_ret );
    }
    other = BENewLabel();
    CGSelOther( sel, other );
    CGSelect( sel, XPop() );
    CGControl( O_LABEL, NULL, other );
    BEFiniLabel( other );
    // mark all referenced statements
    FCodeSeek( curr_obj );
    num_alts = GetU16();
    while( num_alts-- > 0 ) {
        RefStmtLabel( GetPtr() );
    }
}


void    FCAssignAltRet( void )
//============================
// Assign alternate return value to result of subroutine.
{
    cg_name     alt_ret;

    alt_ret = GetTypedValue();
    if( EntryWithAltRets() ) {
        CGDone( CGAssign( CGFEName( ReturnValue, TY_INTEGER ), alt_ret, TY_INTEGER ) );
    } else {
        CGDone( alt_ret );
    }
}
