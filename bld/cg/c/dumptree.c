...
/*************************************************/
    ...
    DumpStrType( node, Ops[node->u1.t.op], Null, indent );
}
...
/***********************************************/
    ...
    for( scan = what->u1.t.rite; scan != NULL; scan = scan->u1.t.rite ) {
        DumpIndent( indent );
        DumpLiteral( "<O_PARM>" );
        DumpNL();
...
/**************************************************/
    ...
    case TN_SIDE_EFFECT:
        ...
        DumpSubTree( node->u1.t.rite, indent+2 );
        break;
    case TN_LV_ASSIGN:
        DumpSubTree( node->u.left, indent+2 );
    ...
    case TN_PRE_GETS:
        DumpSubTree( node->u.left, indent+2 );
        DumpStrType( node, Ops[node->u1.t.op], Eq, indent );
        DumpSubTree( node->u1.t.rite, indent+2 );
        break;
    case TN_LV_PRE_GETS:
        DumpSubTree( node->u.left, indent+2 );
        DumpStrType( node, Ops[node->u1.t.op], LvEq, indent );
        DumpSubTree( node->u1.t.rite, indent+2 );
        break;
    case TN_POST_GETS:
        DumpSubTree( node->u.left, indent+2 );
        DumpStrType( node, Ops[node->u1.t.op], PostEq, indent );
        DumpSubTree( node->u1.t.rite, indent+2 );
        break;
    case TN_FLOW:
    ...
    case TN_QUESTION:
        ...
        DumpSubTree( node->u1.t.rite->u.left, indent+2 );
        DumpStrType( node, Colon, Null, indent );
        DumpSubTree( node->u1.t.rite->u1.t.rite, indent+2 );
        break;
    case TN_BIT_LVALUE:
    case TN_BIT_RVALUE:
        DumpIndent( indent );
        ...
        DumpInt( node->u1.b.start );
        DumpLiteral( "for " );
        DumpInt( node->u1.b.len );
        DumpNL();
        DumpSubTree( node->u.left, indent+2 );
        break;
    case TN_CALLBACK:
