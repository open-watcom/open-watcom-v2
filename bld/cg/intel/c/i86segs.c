...
/*********************************************************************/
    ...
    if( class == CG_TBL || class == CG_VTB || class == CG_CLB ) {
        if( _IsTargetModel( FLAT_MODEL ) ) return( HW_DS );
        return( HW_CS );
    }
    if( class == CG_FE ) {
        attr = FEAttr( sym );
    ...
    if( reg != NULL && !HW_CEqual( *reg, HW_EMPTY ) ) {
        if( HW_COvlap( *reg, HW_SEGS ) ) return( *reg );
        FEMessage( MSG_BAD_PEG_REG, AskForLblSym( CurrProc->label ) );
    }
    if( class == CG_FE && ( attr & FE_PROC ) != EMPTY ) {
#if _TARGET & _TARG_80386
        if( _IsTargetModel( FLAT_MODEL ) ) return( HW_CS ); /* all have same CS */
        if( AskCodeSeg() == id ) return( HW_CS );
#endif
    } else {
        if( _IsTargetModel(FLAT_MODEL) && _IsntTargetModel(FLOATING_DS) ) return(HW_DS);
        if( AskCodeSeg() == id ) {
            /* COMDAT's might be allocated by some other module */
            if( attr & FE_COMMON ) return( HW_EMPTY );
            return( HW_CS ); /* 89-09-01 */
        }
        if( AskSegPrivate( id ) ) return( HW_EMPTY );
        if( _IsntTargetModel( FLOATING_DS ) ) return( HW_DS );
        if( !_RoutineIsInterrupt( CurrProc->state.attr ) &&
             _IsntTargetModel( FLOATING_SS ) ) return( HW_SS );
    }
    return( HW_EMPTY );
}
...
/*************************************/
    ...
    if( HW_CEqual( segreg, HW_CS ) ) return( TRUE );
    return( FALSE );
}
...
/*************************************/
    ...
    segreg = CalcSegment( op->v.symbol, op->m.memory_type );
    if( HW_CEqual( segreg, HW_SS ) ) return( TRUE );
...
/*****************************************/
    ...
    if( !HW_CEqual( reg, HW_EMPTY ) ) return( AllocRegName( reg ) );
            ...
            if( AskNameCode( sym, op->m.memory_type ) ) {
                if( op->m.memory_type == CG_FE ) {
                    if( FEAttr( sym ) & FE_PROC ) {
                        if( IsFarFunc( sym ) ) return( T_LONG_CODE_PTR );
...
/*************************************/
    ...
    if( op->n.class == N_INDEXED ) {
        if( op->i.index->n.class != N_CONSTANT
            && op->i.index->n.size != WORD_SIZE ) return( TRUE );
        if( op->i.base != NULL ) return( SegOver( op->i.base ) );
        if( _IsTargetModel( FLOATING_DS ) ) return( TRUE );
...
*/
    ...
    if( opnd->n.class != N_MEMORY ) return( FALSE );
    segreg = CalcSegment( opnd->v.symbol, opnd->m.memory_type );
    if( HW_CEqual( segreg, HW_SS ) && _IsntTargetModel( FLOATING_SS ) ) return( FALSE );
    if( HW_CEqual( segreg, HW_DS ) && _IsntTargetModel( FLOATING_DS ) ) return( FALSE );
    if( HW_CEqual( segreg, HW_CS ) ) return( FALSE );
    return( TRUE );
}
