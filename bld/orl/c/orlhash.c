...
orl_return ORLHashTableInsert( orl_hash_table hash_table, orl_hash_value key, orl_hash_data data )
{
    ...
    if( !new_data_element ) return( ORL_OUT_OF_MEMORY );
    new_data_element->data = data;
    new_data_element->next = NULL;
    switch( hash_table->type ) {
        case ORL_HASH_STRING:
            hash_value = orlHash( hash_table, orlStringEncode( (char *) key ) );
            break;
        case ORL_HASH_NUMBER:
            hash_value = orlHash( hash_table, key );
            break;
    }
    ...
    if( !new_element ) return( ORL_OUT_OF_MEMORY );
    new_element->key = key;
...
orl_hash_data_struct *ORLHashTableQuery( orl_hash_table hash_table, orl_hash_value key )
{
    ...
    hash_ptr = hash_table->table[hash_value];
    while( hash_ptr != NULL ) {
        if( !( _HashCompare( hash_table, hash_ptr->key, key ) ) ) return( hash_ptr->data_struct );
        hash_ptr = hash_ptr->next;
    }
    return( NULL );
}
