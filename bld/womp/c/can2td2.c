...
/**/myassert( fix->self_relative == 0 );  /* no self-relative */
    ...
    case FIX_BASE:
        codeOffset = 0;
        codeSeg = fix->lr.target_datum + MGET_LE_S16( seg->data );
        break;
    case FIX_POINTER:
        codeSeg = fix->lr.target_datum + MGET_LE_S16( seg->data + 2 );
        codeOffset = fix->lr.target_offset + MGET_LE_S16( seg->data );
        break;
    case FIX_POINTER386:
        codeSeg = fix->lr.target_datum + MGET_LE_S16( seg->data + 4 );
        codeOffset = fix->lr.target_offset + MGET_LE_S32( seg->data );
        break;
    default:
/**/    never_reach();
    }
...
/**/myassert( name_len < 256 );
    ...
    case FIX_POINTER:
    case FIX_OFFSET:
        offset += MGET_LE_S16( mem_loc->data );
        break;
    case FIX_POINTER386:
    case FIX_OFFSET386:
        offset += MGET_LE_S32( mem_loc->data );
        break;
    }
    if( offset > 0xffff ) {
