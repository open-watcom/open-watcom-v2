...
typedef enum {
    ...
    OBJFMT_EASY_OMF             = 0x0001,
    OBJFMT_MS_386               = 0x0002,
    OBJFMT_IS_LIDATA            = 0x0004,   // true if last data was lidata
    OBJFMT_TOLD_BITNESS         = 0x0008,   // already said object was 16/32/64-bit
    OBJFMT_DEBUG_COMENT         = 0x0010,   // saw an object debug coment.
    /* bits 5..7 reserved for FMT_xxxx symbols (for deciding .obj format) */
    /* bits 8..max available */
    OBJFMT_IGNORE_FIXUPP        = 0x0100,   // don't process a fixup record
    OBJFMT_UNSAFE_FIXUPP        = 0x0200,   // don't optimize calls in current fixupp
} obj_format;

#define OBJFMT_32BIT_REC        (OBJFMT_EASY_OMF | OBJFMT_MS_386)
