 ...
 */
{
    ...
    if( type != ORL_SEC_TYPE_NO_BITS
      && type != ORL_SEC_TYPE_PROG_BITS ) {
         return( ORL_OKAY );
    }
    ...
    if( flags & ORL_SEC_FLAG_EXEC ) {
        ...
        namelen = strlen( name );
        if( namelen >= 3
          && strnicmp( name + namelen - 3, "bss", 3 ) == 0 ) {
...
/************************************************/
{
    ...
    if( ImpExternalName == NULL ) {
        if( ImpOrdinal == 0 ) {
            ...
            HandleImport( &intname, &modname, NULL, ImpOrdinal );
        }
    } else {
        length_name     extname;
