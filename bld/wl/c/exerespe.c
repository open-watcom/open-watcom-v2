...
static WResStatus WritePad( int handle, int size )
{
    handle = handle;
    PadLoad( size );
    return WRS_OK;
...
static WResStatus CopyData( int from, int to, int len )
{
    to = to;
    while( len > MAX_HEADROOM ) {
        WRESREAD( from, TokBuff, MAX_HEADROOM );
        WriteLoad( TokBuff, MAX_HEADROOM );
        len -= MAX_HEADROOM;
    }
    if( len > 0 ) {
        WRESREAD( from, TokBuff, len );
        WriteLoad( TokBuff, len );
    }
    return WRS_OK;
}
...
/***************************************************************/
{
    ...
    handle = handle;
    WriteLoad( &entry->Head, sizeof(resource_dir_header) );
    for( child_num = 0; child_num < entry->Head.num_name_entries +
                    entry->Head.num_id_entries; child_num++ ) {
        WriteLoad( entry->Children + child_num, sizeof(resource_dir_entry) );
    }
    return( WRS_OK );
} /* writeDirEntry */
...
{
    handle = handle;
    WriteLoad( &entry->Entry, sizeof(resource_entry) );
    return( WRS_OK );
} /* writeDataEntry */
...
/*****************************************************/
{
    ...
    if( dir->String.StringBlock != 0 ) {
        WriteLoad( dir->String.StringBlock, dir->String.StringBlockSize );
    }
...
WResStatus FiniPEResources( PEResDir *outres )
/********************************************/
{
    WriteDirectory( outres, 0 );
    FreePEResDir( outres );
    return WRS_OK;
}
