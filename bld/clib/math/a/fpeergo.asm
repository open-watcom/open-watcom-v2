...
;
; FPEERGO       : connect/disconnect 80x87 interrupt handler under Ergo
;
...
defp    __Ergo_hook_init_
        ...
        _endif                          ; endif
        mov     BX,CS                   ; set DS:EDX pointing to handler
        mov     DS,BX                   ; ...
        int     21h                     ; set it
        pop     DS                      ; restore DS
        ret                             ; return
endproc __Ergo_hook_init_

;
;/* Disconnect */
;

        xdefp __Ergo_hook_fini_
defp    __Ergo_hook_fini_
        push    DS                      ; save DS
        mov     AH,25h                  ; set old FPE handler
        mov     AL,__IRQ_int            ; get interrupt number
        mov     EDX,__PMAddr            ; set DS:EDX pointing to old handler
        mov     DS,__PMSeg              ; ...
        int     21h                     ; restore previous int handler
        pop     DS                      ; restore DS
        ret                             ; return
endproc __Ergo_hook_fini_
