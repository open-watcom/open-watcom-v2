...
static FILE *PP_Open( const char *filename )
{
    ...
    if( handle != NULL ) {
        prev_file = PP_File;
        PP_File = (FILELIST *)PP_Alloc( sizeof( FILELIST ) );
        if( PP_File == NULL ) {
            fclose( handle );
            handle = NULL;
            PP_File = prev_file;
...
void PPENTRY PP_IncludePathAdd( incl_type incltype, const char *path_list )
{
    ...
    if( path_list != NULL && *path_list != '\0' ) {
        ...
        if( old_list == NULL ) {
            p = new_list = PP_Alloc( len + 1 );
        } else {
            old_len = strlen( old_list );
            new_list = PP_Alloc( old_len + 1 + len + 1 );
...
void PPENTRY PP_IncludePathInit( incl_type incltype )
{
    switch( incltype ) {
    case PPINCLUDE_SYS:
        IncludePath2 = PP_Alloc( 1 );
        *IncludePath2 = '\0';
        break;
    case PPINCLUDE_USR:
    case PPINCLUDE_SRC:
    default:
        IncludePath1 = PP_Alloc( 1 );
        *IncludePath1 = '\0';
        break;
    }
}
...
void PPENTRY PP_Define( const char *ptr )
{
    ...
    if( me != NULL ) {
        ...
        me->replacement_list = PP_Alloc( len );
...
void PPENTRY PP_Define_1( const char *ptr )
{
    ...
    if( me != NULL ) {
        me->parmcount = 0;
        me->replacement_list = PP_Alloc( sizeof( "1" ) );
...
static void IncLevel( int value )
{
    ...
    cpp = (CPP_INFO *)PP_Alloc( sizeof( CPP_INFO ) );
...
/*******************************************************/
{
    ...
    PPLineBuf = PP_Alloc( PPLineBufSize + 2 );
    PPLineBuf[0] = '\0';
    PPLineBuf[1] = '\0';
    PPPreProcChar = c;
