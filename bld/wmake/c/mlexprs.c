 ...
 */
{
    ...
    CurAttr.u.ptr = MemStrdupSafe( file );
    return( tok );
}
 ...
 */
{
    ...
    CurAttr.u.ptr = MemStrdupSafe( ext );
    return( ret );
}
#ifdef __WATCOMC__
#pragma pop( check_stack );
 ...
 */
{
    ...
    /*
     * not a macro line, put everything back
     */
    UnGetCHR( s );
    if( ws ) {
        UnGetCHR( ' ' );
    }
    InsString( MemStrdupSafe( mac + 1 ), true );
    return( false );
}
#ifdef __WATCOMC__
#pragma pop( check_stack );
        ...
        for( current = p; *current != NULLCHAR; ++current ) {
            if( *current == '\"' ) {
                if( !in_dquote ) {
                    ...
                    if( current != p ) {
                        UnGetCHR( STRM_MAGIC );
                        InsString( MemStrdupSafe( current ), true );
                        UnGetCHR( STRM_TMP_LEX_START );
                        *current = NULLCHAR;
                        return( p );
                    }
 ...
 */
{
    ...
    for( ;; ) {
        ...
        if( atstart ) {
            ...
            if( sisws( s ) ) {
                /*
                 * cmd line starts with space
                 */
                return( lexCmd() );
            }
            if( sismacc( s )
              && checkMacro( s ) ) {  /* check if macro = body */
                s = PreGetCHR();
                continue;
            }
