...
/******************************************************************/
{
    ...
    } else if( !(targ->attr.precious
      || targ->attr.symbolic) ) {
        if( !Glob.hold
          && targExists( targ ) ) {
            if( Glob.erase
              || GetYes( SHOULD_FILE_BE_DELETED ) ) {
 ...
 */
{
    getDate( targ );
    if( targ->existing
      && targ->attr.existsonly ) {
        return( true );
    }
    getDate( deptarg );
    if( targ->existing
      && deptarg->existing
      && deptarg->attr.existsonly ) {
        return( true );
    }
 ...
 */
{
    ...
    for( ; cur != NULL; cur = cur->next ) {
        ...
        for( curslist = cur->slist; curslist != NULL && !ok; curslist = curslist->next ) {
            ...
            if( *curslist->targ_path == NULLCHAR
              && *curslist->dep_path == NULLCHAR ) {
...
/*********************************************/
{
    ...
    for( tlist = cur.dep->targs; tlist != NULL; tlist = tlist->next ) {
        targ = tlist->target;
        if( !younger
          || dateCmp( cur.targ->date, targ->date ) < 0 ) {
            if( written ) {
                WriteVec( vec, " " );
            }
