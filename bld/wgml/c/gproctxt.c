...
static void wgml_tabs( void )
{
    ...
    bool                        scope_end   = false;    // spaces at end of ALIGN_center or ALIGN_right scope
    ...
    if( t_count == 0 ) {            // first character is tab character
            ...
            if( t_page.cur_width > (t_line->last->x_address + t_line->last->width) ) {
                if( t_line->last->count == 0 ) {
                    if( phrase_start ) {    // add new marker
                        ...
                        if( c_stop->alignment == ALIGN_center ) {
                            pre_width /= 2;
                            if( (pre_width % 2) > 0 ) {
                                pre_width++;
                            }
        ...
        if( tab_chars.last != NULL ) {
            ...
            if( tab_space > 0 ) {   // tab followed by spaces then text
                if( c_stop->alignment == ALIGN_left ) {   // alignment left
                    s_width = 0;
    ...
    for( ; i < in_count; i++ ) {
        ...
        if( skip_tab ) {    // never true for ALIGN_left
            ...
            if( s_chars->prev != NULL ) {
                if( ((s_chars->prev->type == TXT_norm)
                  && (c_type != TXT_norm))
                  || (s_chars->prev->type != TXT_norm)
                  && (c_type == TXT_norm)) {
            ...
            if( s_chars->prev != NULL ) {
                if( (s_chars->type == TXT_norm) && (c_type != TXT_norm) ) {
                    c_chars = do_c_chars( c_chars, in_chars, NULL, 0,
                                        t_page.cur_width, 0, c_font, c_type );
                    if( tab_chars.first == NULL ) {
                        tab_chars.first = c_chars;
            ...
            if( (t_width > 0) || (c_stop->alignment != ALIGN_center) || !pre_tab_gap ) {
                ...
                if( s_multi == NULL ) { // only first token in tab scope
                    if( def_tab_count > 0 ) {
                        c_chars->tab_pos = TAB_def;
                    } else {
                        c_chars->tab_pos = TAB_user;
                    }
    ...
    if( (in_text[i] == '\t') || (in_text[i] == tab_char) ) {
        ...
        if( c_stop->alignment == ALIGN_right ) {
        ...
        if( !ProcFlags.in_figlist_toc
          && (s_chars->prev != NULL) ) {    // Skip for TOC output
            if( ((s_chars->prev->type == TXT_norm)
              && (c_type != TXT_norm))
              || (s_chars->prev->type != TXT_norm)
              && (c_type == TXT_norm)) {
    ...
    while( cur_chars != NULL ) {
        ...
        case ALIGN_right:
            if( c_stop->column < (scope_width + tab_chars.first->ts_width) ) {
                ...
                if( gap_start < (c_stop->column + tab_col - (scope_width + tab_chars.first->ts_width)) ) {
        ...
        if( skip_tab ) {    // never true for ALIGN_left
            ...
            if( def_tab_count > 0 ) {
                tab_chars.first->tab_pos = TAB_def;
            } else {
...
//    if( (sum_w <= 0) || (hor_end >= rm) || (line_width < 1) ) {
    ...
    if( (input_cbs->fmflags & II_research)
      && GlobalFlags.lastpass ) {
        find_symvar( sys_dict, "$ju", SI_no_subscript, &symjusub);// .ju as string
#if 0
...
void process_line_full( text_line * a_line, bool justify )
{
      ...
      && (a_line->first->next != NULL) ) {
        ...
        if( a_line->last->count == 0 ) {
            ...
            while( (split_chars != NULL) ) {
                ...
                if( split_chars->tab_pos == TAB_none ) { // ordinary token found
                    break;
                }
                split_chars = split_chars->prev;
            }
        ...
        if( (split_chars != NULL)
          && (split_chars->prev != NULL) ) {    // don't split if a_line would be left empty
            ...
            if( (b_line->first->count > 0)
              || (b_line->first->next != NULL) ) {
                ...
                if( !split_chars->pre_gap ) {
                    split_chars->tab_pos = TAB_none;     // not tabbed any longer if ever was
                }
    ...
    while( a_line != NULL ) {
        ...
        if( t_element == NULL ) {
            ...
            t_element = init_doc_el( ELT_text, a_line->line_height );
...
void process_text( char * text, font_number font )
{
    ...
    if( tab_char == '\t' ) {                        // ignore keyboard tabs
        ...
        if( (tt_stack != NULL)
          && (!input_cbs->fm_hh)
          && (*p == tab_char) ) {   // adjust parent font, see Wiki
    ...
    phrase_start = true;
    if( !ProcFlags.ct
      && !ProcFlags.in_figlist_toc ) {  // not if text follows CT or in TOC/FIGLIST
    ...
    if( t_line->first == NULL ) {    // first phrase in line
        if( ProcFlags.concat ) {
            ...
            while( (*p == '\t') || (*p == ' ') || (*p == '\0') ) {
                if( *p == '\0' )
                    break;                  // done at end of input text fragment
                if( *p == ' ') {
            ...
            if( ProcFlags.concat ) {   // CO on and not LI
                if( post_space == 0 ) {
                    ...
                    if( ProcFlags.wrap_indent
                      && (font != g_prev_font) ) { // font changed (INDEX see output)
    ...
    while( *p != '\0' ) {
        ...
        if( *p != '\0' ) {          // process last word inside loop
            ...
            if( *p != FUNC_escape ) {   // FUNC_escape ends word, even with CO OFF
                ...
                if( !ProcFlags.concat
                  && (cur_group_type != GRT_xmp) ) { // .co off but not XMP: include internal spaces
                    continue;
                }
        ...
        if( n_chars == NULL ) {
            // can happen at end of phrase or after FUNC_escape sequence
            ...
            n_chars = process_word( pword, count, font,
                    ProcFlags.null_value && (!ProcFlags.concat || (cur_group_type == GRT_xmp)) );
            n_chars->type = typ;
            typ = typn;
            typn = TXT_norm;             // reset for next text_chars
        }
        ...
        if( (t_line != NULL)
          && (t_line->last != NULL) ) {
            if( (t_line->last->font == n_chars->font)
              || !ProcFlags.post_ix ) {
                keep_together = true;
            }
        }
        ...
        line_split = false;
        if( ProcFlags.concat
          || (cur_group_type == GRT_xmp) ) { // .co on or XMP
            ...
            while( !tabbing &&
                ...
                if( (t_line->last != NULL)
                  ...
                  && keep_together)
                  || (t_line->last->type & TXT_sub)
                  || (t_line->last->type & TXT_sup)) ) {

                    /* Initialize s_chars */

                    s_chars = t_line->last;
                    while( (t_page.cur_width == (s_chars->x_address + s_chars->width))
                            || ((s_chars->type & TXT_sub)
                            || (s_chars->type & TXT_sup)) ) {
            ...
            if( *p == ' ' ) {           // spaces to process
                ...
                if( (cur_group_type == GRT_xmp)
                  ...
                  || (*p == tab_char)) ) {
                    ...
                    if( (*pword == '\t')
                      || (*pword == tab_char) ) {    // no text after tab char
                    ...
                    post_space = wgml_fonts[font].spc_width;
                    if( !ProcFlags.as_text_line
                      && stop_fnd
                      && (cur_group_type != GRT_xmp) ) {   // exclude XMP
                        post_space *= 2;
                    }
                    p++;
                    pa = p;
