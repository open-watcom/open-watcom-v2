    ...
    *pw = '\0';
    ...
    if( old_errs == err_count ) {
        ...
        if( new_font->name !=NULL ) {
            MemFree( new_font->name );
            new_font->name = NULL;
        }
        if( new_font->style !=NULL ) {
            MemFree( new_font->style );
            new_font->style = NULL;
        }
        MemFree( new_font );
        new_font = NULL;
...
static void set_outfile( option * opt )
{
    ...
    out_file = MemToStringSafe( tokennext->token, tokennext->toklen );
...
static cmd_tok * process_master_filename( cmd_tok * tok )
{
    ...
    p = MemToStringSafe( tok->token, tok->toklen );
...
int proc_options( char * string )
{
    ...
    for( ;; ) {
        ...
        if( cmd_data_lvl->file_name != NULL ) {
            MemFree( cmd_data_lvl->file_name );
