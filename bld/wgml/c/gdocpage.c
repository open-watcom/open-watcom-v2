    ...
    for( i = 0; i < count; i++ ) {
        ...
        while( cur_el != NULL ) {
            if( i == 0 ) {      // restrict output to first column, for now
                ...
                case el_graph :
                    if( FlagsGlob.lastpass ) {
                        if( FlagsProc.ps_device ) {   // only available to PS device
...
static void set_v_positions( doc_element * list, spacing_bu v_start )
{
    ...
    for( cur_el = list; cur_el != NULL; cur_el = cur_el->next ) {
        ...
        switch( cur_el->type ) {
        case el_binc :
            cur_el->element.binc.at_top = !FlagsProc.page_started &&
        ...
        case el_dbox :
            ...
            break;
        case el_graph :
            cur_el->element.graph.at_top = !FlagsProc.page_started &&
                                          (t_page.top_banner == NULL);
            if( bin_driver->y_positive == 0x00 ) {
                g_cur_v_start -= cur_spacing;
            } else {
        ...
        case el_text :
            for( cur_line = cur_el->element.text.first; cur_line != NULL;
                ...
                if( FlagsProc.page_started ) {          // not first element
    ...
    while( n_page.col_main != NULL ) {
        ...
        if( cur_el->blank_lines > 0 ) {
            if( (t_page.cur_depth + cur_el->blank_lines) >= t_page.max_depth ) {
                ...
                break;
            } else if( !FlagsProc.page_started
...
// these were in document_top_banner()
    ...
    if( apage && FlagsGlob.lastpass ) {   // don't do before first page
...
void insert_col_fn( doc_element * a_element )
{
    ...
    if( !FlagsProc.page_started ) { // first element ignores pre_skip TBD
        depth = a_element->top_skip;
    } else {
        depth = a_element->subs_skip;
    }
