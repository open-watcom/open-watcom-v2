...
static void gml_xl_lp_common( gml_tag gtag )
{
    ...
    if( gtag != GML_TAG_LP ) {                       // text only allowed for :LP
        if( gtag != GML_TAG_DL && gtag != GML_TAG_GL ) {      // DL/GL don't require LI/LP
            FlagsProc.need_li_lp = true;    // :LI or :LP  next
        }
...
static void    gml_exl_common( gml_tag egtag )
{
    ...
    FlagsProc.need_li_lp = false;        // :LI or :LP no longer needed
    scan_start = scan_stop;
}
...
static  void    gml_li_ol( gml_tag gtag )
{
    ...
    if( FlagsProc.need_li_lp ) {        // first :li for this list
    ...
    FlagsProc.need_li_lp = false;       // 1. item in list processed
    if( *p ) {
...
void    gml_lp( gml_tag gtag )
{
    ...
    if( FlagsProc.need_li_lp ) {        // :LP first tag in list
    ...
    if( *p == '.' ) p++;                // over '.'
    while( *p == ' ' ) p++;             // skip initial spaces
    FlagsProc.need_li_lp = false;       // :LI or :LP seen
    if( *p ) {
        process_text( p, g_curr_font ); // if text follows
    }
