...
void gml_figref( const gmltag * entry )
{
    ...
    if( cur_re == NULL ) {              // undefined refid
        if( do_page ) {
            ref_text = MemAllocSafe( dr_len + dp_len + 1 );
            strcpy( ref_text, def_ref );
            strcat( ref_text, def_page );
        } else {
            ref_text = MemAllocSafe( dr_len + 1 );
            strcpy( ref_text, def_ref );
        }
    } else {
        ...
        if( do_page ) {
            ...
            ref_text = MemAllocSafe( len + op_len + bu_len  + 1 );
...
void gml_hdref( const gmltag * entry )
{
    ...
    op_len = strlen( on_page );
    if( cur_re == NULL ) {              // undefined refid
        ref_text = MemAllocSafe( dp_len + dr_len + 1 );
        strcpy( ref_text, def_ref );
        if( do_page ) {
          strcat( ref_text, def_page );
        }
