...
void  scr_process_break( void )
{
    if( t_line != NULL ) {
        if( t_line->first != NULL ) {
            ...
            process_line_full( t_line, ((FlagsProc.justify != ju_off) &&
                (FlagsProc.justify != ju_on) && (FlagsProc.justify != ju_half)) );
            t_line = NULL;
        }
    }
    if( t_element != NULL ) {
        ...
        t_element = NULL;
        t_el_last = NULL;
    } else if( FlagsProc.empty_doc_el ) {   // empty element needed?
