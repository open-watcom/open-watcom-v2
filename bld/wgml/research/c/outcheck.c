...
static oc_element * oc_alloc_oc_element( oc_element_type el_type )
{
    ...
    if( oc_element_pool == NULL ) {
        retval = MemAllocSafe( sizeof( *retval ) );
    } else {
...
static text_chars *oc_alloc_text_chars( char *in_text, size_t count, font_number font, text_type type )
{
    ...
    if( oc_text_chars_pool == NULL ) {
        ...
        retval = MemAllocSafe( sizeof( *retval ) + size );
        retval->length = size;
    } else {
        ...
        if( count > retval->length ) {
            size = (( count / TEXT_START ) + 1) * TEXT_START;
            retval = MemReallocSafe( retval, sizeof( *retval ) + size );
            retval->length = size;
        }
    }
    retval->next = NULL;
...
static uint32_t oc_tab_position( uint32_t cur_pos )
{
    ...
    if( cur_pos > last_tab ) {
        ...
        if( (def_tabs.tabs[def_tabs.current - 1].column + req_count) > def_tabs.length ) {
            ...
            def_tabs.tabs = MemReallocSafe( def_tabs.tabs, sizeof( *def_tabs.tabs ) * req_length );
...
static void emulate_wgml( void )
{
    ...
    oc_text_chars_pool = MemAllocSafe( sizeof( *oc_text_chars_pool ) + TEXT_START );
    ...
    for( i = 0; i < 19; i++ ) {
        tc_pool_ptr->next = MemAllocSafe( sizeof( *tc_pool_ptr ) + TEXT_START );
    ...
    oc_text_line_pool = MemAllocSafe( sizeof( *oc_text_line_pool ) );
    ...
    for( i = 0; i < 9; i++ ) {
        tl_pool_ptr->next = MemAllocSafe( sizeof( *tl_pool_ptr ) );
    ...
    oc_element_pool = MemAllocSafe( sizeof( *oc_element_pool ) );
    ...
    for( i = 0; i < 9; i++ ) {
        te_pool_ptr->next = MemAllocSafe( sizeof( *te_pool_ptr ) );
...
{
    puts( banner1t( "Document Output Test Program" ) "\n"
