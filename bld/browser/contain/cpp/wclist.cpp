...
WCSLink * WCIsvListBase::base_next( const WCSLink * curr, WCbool reset ) const {
    if( reset ) {
        curr = tail;
        if( curr == 0 ) {
            return( 0 );
...
void WCIsvListBase::base_insert( WCSLink * datum ) {
    if( tail == 0 ) {
        // link datum to itself
        datum->link = datum;
        tail = datum;
    } else {
...
int WCIsvListBase::base_index_check( int index ) const {
...
WCSLink * WCIsvListBase::base_bfind( int index ) const {
...
WCSLink * WCIsvListBase::base_sfind( int index ) const {
...
WCDLink * WCIsvListBase::base_dfind( int index ) const {
    int entry_index = entry_count - 1;
...
WCSLink * WCIsvListBase::base_sget( int index ) {
...
WCDLink * WCIsvListBase::base_dget( int index ) {
...
int WCIsvListBase::base_index( const WCSLink * datum ) const {
    WCSLink * rover;
    int index = 0;
...
void WCIsvListBase::base_clear( void ) {
    tail = 0;
    entry_count = 0;
};
...
void WCIsvListBase::base_destroy( void ) {
    WCSLink * rover;
    WCSLink * next_link;
...
WCSLink * WCListIterBase::base_advance( int adv_amount ) {
...
WCDLink * WCListIterBase::base_retreat( int adv_amount ) {
...
WCSLink * WCListIterBase::base_tail_hit( WCSLink * new_tail ) {
    WCSLink * list_tail = curr_list->tail;
...
void WCListIterBase::base_tail_unhit( WCSLink * old_tail ) {
    if( old_tail != 0 ) {
