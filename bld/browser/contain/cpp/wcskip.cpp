...
// Skip List Random Number Implementation
...
unsigned WCSkipNonTempBase::randomsLeft = WCSkipBitsInRandom / 2;
unsigned WCSkipNonTempBase::randomBits = rand();



WCSkipNonTempBase::WCSkipNonTempBase( unsigned prob, unsigned max_ptrs
                 , WCbool duplicates )
          : alloc_fn( 0 ), dealloc_fn( 0 )
          , level( 0 ), allowDuplicates( duplicates ), num_entries( 0 ) {
...
void WCSkipNonTempBase::base_assign( const WCSkipNonTempBase * orig ) {
    if( this != orig ) {
        clear();
...
void WCSkipNonTempBase::base_construct( const WCSkipNonTempBase * orig ) {
    ...
    curr = orig->header->forward[ 0 ];
    while ( curr != 0 ) {
        if( !base_insert_copy( curr ) ) {
            break;
        }
        curr = curr->forward[ 0 ];
    }
...
WCSkipListPtrs * WCSkipNonTempBase::base_find( TTypePtr elem ) const {
    register int k;
    register node_ptr curr, next;
    curr = header;
    k = level;
    do {
        while( next = curr->forward[ k ]
             , ( ( next != 0 )&&( base_less( next, elem ) ) ) ) {
            curr = next;
        }
    } while( --k >= 0 );
...
WCSkipListPtrs * WCSkipNonTempBase::base_find_with_update( TTypePtr elem
...
void WCSkipNonTempBase::base_init( unsigned prob, unsigned max_ptrs ) {
    ...
    if( ( prob != WCSKIPLIST_PROB_QUARTER )
      &&( prob != WCSKIPLIST_PROB_HALF ) ) {
...
WCSkipListPtrs * WCSkipNonTempBase::base_insert( TTypePtr elem ) {
    int k;
    node_ptr curr, found, new_node;
...
unsigned WCSkipNonTempBase::base_occurrencesOf( TTypePtr elem ) const {
    unsigned count = 0;
...
int WCSkipNonTempBase::base_random_level() {
...
unsigned WCSkipNonTempBase::base_removeAll( TTypePtr elem ) {
    int k, m, i;
    node_ptr curr, next, delete_node;
    unsigned count = 0;
    node_ptr update[ WCSKIPLIST_MAX_PTRS ];
    ...
    if( delete_node ) {
        ...
        while( ( ( next = last_delete_node->forward[ 0 ] ) != 0 )
             &&( base_equiv( next, elem ) ) ) {
...
WCSkipListPtrs *WCSkipNonTempBase
                        ::base_remove_but_not_delete( TTypePtr elem
...
WCbool WCSkipNonTempBase::remove( TTypePtr elem ) {
    int num_ptrs_in_node;
