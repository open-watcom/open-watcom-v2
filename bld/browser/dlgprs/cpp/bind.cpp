...
//---------------------------------------------------------------------
{
    ...
    va_start( args, fmt );
    len = std::vsprintf( buffer, fmt, args );
    file.write( buffer, (int)std::strlen( buffer ) );
    va_end( args );
...
//-------------------------------------------------------
{
    ...
    tText = std::ctime( &timeOfDay );
    tText[ std::strlen( tText ) - 1 ] = '\0';        // nuke '\n'
...
//--------------------------------------------------------------------------
{
    ...
    for( i = 0; i < _controls->size(); i += 1 ) {
        ...
        if( dlgControl ) {
            ...
            std::printf( "Error: couldn't find dialog control for %s\n", ctrl->getText() );
