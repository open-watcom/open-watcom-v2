...
public:
    ...
    using LocalPointerBase<UConverter>::operator->;
    LocalUConverterPointer(UConverter *p=NULL) : LocalPointerBase<UConverter>(p) {}
    LocalUConverterPointer(LocalUConverterPointer &&src) U_NOEXCEPT
            : LocalPointerBase<UConverter>(src.ptr) {
        src.ptr=NULL;
    }
    ~LocalUConverterPointer() { if (ptr != NULL) { ucnv_close(ptr); } }
    LocalUConverterPointer &operator=(LocalUConverterPointer &&src) U_NOEXCEPT {
        return moveFrom(src);
    }
    LocalUConverterPointer &moveFrom(LocalUConverterPointer &src) U_NOEXCEPT {
        if (ptr != NULL) { ucnv_close(ptr); }
        LocalPointerBase<UConverter>::ptr=src.ptr;
        src.ptr=NULL;
        return *this;
    }
    void swap(LocalUConverterPointer &other) U_NOEXCEPT {
        UConverter *temp=LocalPointerBase<UConverter>::ptr;
        LocalPointerBase<UConverter>::ptr=other.ptr;
        other.ptr=temp;
    }
    friend inline void swap(LocalUConverterPointer &p1, LocalUConverterPointer &p2) U_NOEXCEPT {
        p1.swap(p2);
    }
    void adoptInstead(UConverter *p) {
        if (ptr != NULL) { ucnv_close(ptr); }
        ptr=p;
    }
};
...
public:
    using LocalPointerBase<UConverter>::operator*;
    using LocalPointerBase<UConverter>::operator->;
    explicit LocalUConverterPointer(UConverter *p=NULL) : LocalPointerBase<UConverter>(p) {}
    ~LocalUConverterPointer() { ucnv_close(ptr); }
    LocalUConverterPointer &moveFrom(LocalUConverterPointer &src) U_NOEXCEPT {
        if (ptr != NULL) { ucnv_close(ptr); }
        LocalPointerBase<UConverter>::ptr=src.ptr;
        src.ptr=NULL;
