...
WPI_HBITMAP _wpi_selectbitmap( WPI_PRES pres, WPI_HBITMAP hbitmap )
/*****************************************************************/
{
    WPI_OBJECT  *obj;
    WPI_OBJECT  *old_obj;
    ...
    obj = (WPI_OBJECT *)hbitmap;
    if( obj != NULL ) {
        old_obj = MemAllocSafe( sizeof( *old_obj ) );
...
void _wpi_getoldbitmap( WPI_PRES pres, WPI_HBITMAP old_hbitmap )
/**************************************************************/
{
    WPI_OBJECT  *old_obj;

    old_obj = (WPI_OBJECT *)old_hbitmap;
...
void _wpi_deletebitmap( WPI_HBITMAP hbitmap )
/*******************************************/
{
    WPI_OBJECT  *obj;
...
//    LONG              matched;
    ...
    oldwfont = MemAlloc( sizeof( *oldwfont ) );
    if( oldwfont != NULL ) {
        _wpi_getfontattrs( &fm, oldwfont );
    }

    tmp_wfont = MemAllocSafe( sizeof( *tmp_wfont ) );
    ...
    if( (tmp_wfont)->szFacename == NULL
        ...
        if( GpiCreateLogFont( (WPI_PRES)hps, (PSTR8)NULL,
                1L, (PFATTRS)tmp_wfont ) == FONT_MATCH ) {
            goto found;
        }
        GpiDeleteSetId( hps, 1L );
        goto notfound;
    ...
    if( num_fonts != 0 && num_fonts != GPI_ALTERROR ) {
        pfm = MemAllocSafe( sizeof( *pfm ) * num_fonts );
...
/*********************************/
{
    ...
    null_brush = MemAllocSafe( sizeof( *null_brush ) );
...
HBRUSH _wpi_createpatternbrush( WPI_HBITMAP hbitmap )
/***************************************************/
{
    WPI_OBJECT          *obj;
    WPI_OBJECT          *brush;
    ...
    obj = (WPI_OBJECT *)hbitmap;
    brush = MemAllocSafe( sizeof( *brush ) );
    brush->type = WPI_PATBRUSH_OBJ;
...
/***********************************************/
{
    ...
    new_brush = MemAllocSafe( sizeof( *new_brush ) );
...
/**************************************************/
{
    ...
    old_brush = MemAllocSafe( sizeof( *old_brush ) );
...
WPI_HANDLE _wpi_selectobject( WPI_PRES pres, WPI_HANDLE v_obj )
/*************************************************************/
{
    WPI_OBJECT  *obj;
    WPI_OBJECT  *old_obj;
    ...
    case WPI_PEN_OBJ:
        ...
        GpiQueryAttrs( pres, PRIM_LINE, LBB_COLOR | LBB_MIX_MODE | LBB_WIDTH | LBB_TYPE, &(old_obj->pen) );
        GpiSetAttrs( pres, PRIM_LINE, LBB_COLOR | LBB_WIDTH | LBB_TYPE, 0L, &(obj->pen) );
        break;
    ...
    case WPI_BRUSH_OBJ:
        ...
        GpiSetAttrs( pres, PRIM_AREA, ABB_COLOR | ABB_SYMBOL, 0L, &(obj->brush.info) );
        break;
    ...
    case WPI_NULLBRUSH_OBJ:
        _OldBrush( pres, old_obj );
        GpiSetAttrs( pres, PRIM_AREA, ABB_COLOR | ABB_MIX_MODE, 0L, &(obj->brush.info) );
        break;
...
void _wpi_getoldobject( WPI_PRES pres, WPI_HANDLE v_old_obj )
/***********************************************************/
{
    WPI_OBJECT  *old_obj;
    ...
    switch( old_obj->type ) {
    case WPI_PEN_OBJ:
        GpiSetAttrs( pres, PRIM_LINE, LBB_COLOR | LBB_WIDTH | LBB_TYPE | LBB_MIX_MODE, 0L, &(old_obj->pen) );
        break;

    case WPI_BRUSH_OBJ:
        GpiSetAttrs( pres, PRIM_AREA, ABB_COLOR | ABB_SYMBOL | ABB_MIX_MODE, 0L, &(old_obj->brush.info) );
        break;
    case WPI_PATBRUSH_OBJ:
        GpiSetAttrs( pres, PRIM_AREA, ABB_COLOR | ABB_SET, 0L, &(old_obj->brush.info) );
        break;
...
/*****************************/
{
    ...
    nullpen = MemAllocSafe( sizeof( *nullpen ) );
...
/****************************************************************/
{
    ...
    new_pen = MemAllocSafe( sizeof( *new_pen ) );
...
/********************************************/
{
    ...
    old_pen = MemAllocSafe( sizeof( *old_pen ) );
    pen = (WPI_OBJECT *)obj;
...
LONG _wpi_setbitmapbits( WPI_HBITMAP hbitmap, int size, BYTE *bits )
{
    ...
    ih.cbFix = sizeof( BITMAPINFOHEADER );
    GpiQueryBitmapParameters( obj->bitmap, &ih );
    bmi = MemAllocSafe( sizeof( BITMAPINFO ) + sizeof( RGB ) * ( 1 << ih.cBitCount ) );
    memcpy( bmi, &ih, sizeof( BITMAPINFOHEADER ) );
...
LONG _wpi_getbitmapstruct( WPI_HBITMAP hbitmap, WPI_BITMAP *info )
{
    WPI_OBJECT  *obj;
...
int _wpi_getdibits( WPI_PRES pres, WPI_HBITMAP hbitmap, UINT start, UINT count,
 ...
 */
{
    ...
    old_hbitmap = GpiSetBitmap( pres, obj->bitmap );
    hab = WinQueryAnchorBlock( HWND_DESKTOP );
    ...
    if( ret == -1 ) {
        hdc = DevOpenDC( hab, OD_MEMORY, "*", 5L, (PDEVOPENDATA)&dop, NULLHANDLE );
        memhps = GpiCreatePS( hab, hdc, &sizl, PU_PELS | GPIA_ASSOC );
        old_hbitmap2 = GpiSetBitmap( memhps, obj->bitmap );
        ret = (int)GpiQueryBitmapBits( memhps, (LONG)start, (LONG)count, buffer, info );
        GpiSetBitmap( memhps, old_hbitmap2 );
        GpiDestroyPS( memhps );
        DevCloseDC( hdc );
    }
    GpiSetBitmap( pres, old_hbitmap );
