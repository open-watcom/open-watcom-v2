...
void WToggleInsertSubitems( WMenuEditInfo *einfo )
{
    ...
    if( einfo->insert_subitems ) {
        ChangeToolButtonBitmap( einfo->ribbon->tbar, IDM_MENU_INSERTSUBITEMS,
                                InsertSub );
    } else {
        ChangeToolButtonBitmap( einfo->ribbon->tbar, IDM_MENU_INSERTSUBITEMS,
                                InsertNoSub );
...
void WToggleInsertBitmap( WMenuEditInfo *einfo )
{
    ...
    if( einfo->insert_before ) {
        ChangeToolButtonBitmap( einfo->ribbon->tbar, IDM_MENU_INSERTTOGGLE,
                                InsertBefore );
    } else {
        ChangeToolButtonBitmap( einfo->ribbon->tbar, IDM_MENU_INSERTTOGGLE,
                                InsertAfter );
...
Bool WHandleWM_CLOSE( WMenuEditInfo *einfo, Bool force_exit )
{
    ...
    if( einfo ) {
        ...
        if( ret ) {
            SendMessage( einfo->info->parent, MENU_I_HAVE_CLOSED, 0,
                         (LPARAM) einfo->hndl );
...
void WHandleClear( WMenuEditInfo *einfo )
{
    if( einfo->menu && einfo->menu->first_entry ) {
        if( WQueryClearRes( einfo ) ) {
            ...
            SendDlgItemMessage( einfo->edit_dlg, IDM_MENUEDLIST,
                                LB_RESETCONTENT, 0, 0 );
