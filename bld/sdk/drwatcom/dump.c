...
void DumpTask( HWND hwnd )
{
    ...
    DialogBox( Instance, "DMPTASK", hwnd, fp );
    FreeProcInstance( fp );
    ...
    case WM_COMMAND:
        ...
        case IDOK:
            GetDlgItemText( hwnd, DBG_DEBUGGER_OPT, DebuggerOpts,
                            sizeof( DebuggerOpts )  );
            sprintf( str,"dvw %s %d", DebuggerOpts, currTask );
            WinExec( str, SW_SHOWNORMAL );
            EndDialog( hwnd, 0 );
            break;
        case IDCANCEL:
                                    ...
                                    DWORD lparam )
{
    ...
    case WM_COMMAND:
        ...
        case TASKCTL_DEBUG:
            ...
            DialogBox( Instance, "DEBUGGER_OPTS", hwnd, fp );
            FreeProcInstance( fp );
            break;
#if( 0 )
        case TASKCTL_DUMP:
...
void DumpATask( HWND hwnd )
{
    ...
    DialogBox( Instance, "TASKCTL", hwnd, fp );
    DumpDialogHwnd = 0;
    FreeProcInstance( fp );
