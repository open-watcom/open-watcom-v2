...
static dip_fhandle      curFileHdl;
static BOOL             dipIsLoaded;



dip_fhandle PathOpen( char *name, unsigned len, char *ext ) {
    ...
    if( *path == '\0' ) {
        ...
        return( DIPCliOpen( path, DIP_READ ) );
    }
}
...
BOOL LoadDbgInfo( void ) {
    ...
    curFileHdl = DIPCliOpen( DTModuleEntry.szExePath , DIP_READ );
    if( curFileHdl != -1 ) {
 ...
 *     horkyFindSegment.
 */
void DIPCLIENT DIPCliMapAddr( addr_ptr *addr, void *info ) {
...
dip_fhandle DIPCLIENT DIPCliOpen( char *path, dip_open mode ) {
    ...
    flags = O_BINARY;
    if( mode & DIP_READ )  flags |= O_RDONLY;
    if( mode & DIP_WRITE ) flags |= O_WRONLY;
    if( mode & DIP_TRUNC ) flags |= O_TRUNC;
    if( mode & DIP_CREATE ) {
        flags |= O_CREAT;
        ret = (dip_fhandle)sopen( path, flags, SH_DENYWR,
