...
BOOL GetRetCode( HWND parent, RetCodeTypes type, DWORD id, DWORD *rc ) {
    ...
    info.id = id;
    info.rc = 0;
    DialogBoxParam( Instance, "RET_CODE_DLG", parent, RetCodeDlgProc,
               (DWORD)&info );
    *rc = info.rc;
...
static void fillThreadInfo( HWND hwnd, ProcStats *info ) {
    ...
    if( index == LB_ERR ) {
        ...
        GetThreadInfo( info->pid, threadid, &thdinfo );

        sprintf( buf, "tid = %08lX", threadid );
        SetDlgItemText( hwnd, THREAD_TID, buf );

//here  val = SuspendThread( thread->threadhdl );
//here  if( val == 0xffffffff ) {
//here      SetDlgItemText( hwnd, THREAD_SUSPEND_CNT,
//here                      "Unknown suspend count" );
//here  } else {
//here      ResumeThread( thread->threadhdl );
//here      sprintf( buf, "Suspend count = %ld", val );
//here      SetDlgItemText( hwnd, THREAD_SUSPEND_CNT, buf );
//here  }
        ...
        /* get priority */
        RCsprintf( buf, STR_PRIORITY_X, thdinfo.cur_pri, thdinfo.base_pri );
...
BOOL CALLBACK ThreadCtlProc( HWND hwnd, UINT msg, UINT wparam, DWORD lparam )
{
    ...
    case WM_INITDIALOG:
        ...
        GetProcessInfo( lparam, &info->procinfo );
        info->procid = lparam;
        ThreadDlg = hwnd;
    ...
    case WM_COMMAND:
        ...
        case THREAD_REFRESH:
            RefreshInfo();
            GetProcessInfo( info->procid, &info->procinfo );
            fillThreadCtl( hwnd, &info->procinfo, buf );
            fillThreadInfo( hwnd, &info->procinfo );
            break;
        case THREAD_SUSPEND:
