...
static void fillTaskListBox( HWND hwnd, char *buf ) {
    ...
    if( topindex == -1 ) {
        ...
        for( topindex = 0;; topindex++ ) {
            if( SendMessage( lb, LB_GETITEMDATA, topindex, 0 ) > topproc ) {
                topproc--;
                break;
            }
        }
...
BOOL CALLBACK ProcPriorityDlg( HWND hwnd, UINT msg, UINT wparam, DWORD lparam )
{
    ...
    case WM_INITDIALOG:
        ...
        GetProcessInfo( info->procid, &info->stats );
...
void AddRunningErrMsg( ProcAttatchInfo *info ) {
    ...
    GetProcessInfo( info->info.pid, &stats );
...
BOOL CALLBACK ProcListProc( HWND hwnd, UINT msg, UINT wparam, DWORD lparam )
{
    ...
    case WM_COMMAND:
        ...
        case PROCCTL_SET_PRIORITY:
            {
                ...
                DialogBoxParam( Instance, "PROC_PRIORITY", hwnd,
        ...
        case PROCCTL_ATTATCH:
            ...
            break;
        case PROCCTL_THREAD:
            DialogBoxParam( Instance, "THREAD_CTL", hwnd, ThreadCtlProc,
                            procid );
            break;
        case PROCCTL_KILL:
...
void DisplayProcList( void ) {
    ...
    RefreshCostlyInfo();
    RefreshInfo();
    DialogBox( Instance, "PROCCTL", MainHwnd, ProcListProc );
}
