...
void initDipMsgs( void ) {
    ...
    dipLoadMsgs = MemAlloc( dipCnt * sizeof( char * ) );
}
...
void ShowDIPStatus( HWND hwnd ) {
    ...
    DialogBox( Instance, "DIP_STATUS_DLG", hwnd, fp );
    FreeProcInstance( fp );
}
...
BOOL LoadTheDips( void ) {
    ...
    while( *ptr != '\0' ) {
        ...
        dipLoadMsgs[i] = MemAlloc( strlen( buf ) + 1 );
        strcpy( dipLoadMsgs[i], buf );
        ptr += strlen( ptr ) + 1;
        i++;
    }
    if( !diploaded ) {
