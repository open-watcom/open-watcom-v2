...
/* static variables                                                         */
/****************************************************************************/
static DialogStyle       WdeGenericMask = WS_POPUP        | WS_CHILD        |
...
Bool WdeGenericDefine ( WdeDefineObjectInfo *info )
{
    ...
    if ( ( info->win == NULL ) &&
         !Forward ( info->obj, GET_WINDOW_HANDLE, &info->win, NULL ) ) {
    ...
    proc_inst = (DLGPROC) MakeProcInstance ( (FARPROC) WdeGenericDefineProc,
                                             app_inst );
    ...
    if ( redraw == -1 ) {
        ...
        if ( !Forward ( info->obj, DESTROY_WINDOW, &quick,
                        &destroy_children ) ) {
...
Bool WdeGenericSetDefineInfo ( WdeDefineObjectInfo *info, HWND hDlg )
{
    ...
    if ( !info->obj_id &&
         !Forward ( info->obj, IDENTIFY, &info->obj_id, NULL ) ) {
        WdeWriteTrail("WdeGenericSetDefineInfo: IDENTIFY failed!");
        return ( FALSE );
    }

    if ( info->obj_id == BASE_OBJ ) {
        WdeWriteTrail("WdeGenericSetDefineInfo: "
                      "Attempt to define base object.!");
        return ( FALSE );
        ...
        if( GETHDR_CAPTION(info->info.d.header) != NULL ) {
            WdeSetEditWithStr( GETHDR_CAPTION(info->info.d.header),
                               hDlg, IDB_TEXT );
        }
        ...
        if( GETCTL_TEXT( info->info.c.info ) ) {
            WdeSetEditWithResNameOr( GETCTL_TEXT( info->info.c.info ),
                                     hDlg, IDB_TEXT);
        }
    }
                                      ...
                                      WPARAM wParam, volatile LPARAM lParam )
{
    ...
    if ( info ) {
        if ( info->hook_func ) {
            ret = (*(info->hook_func) ) ( hDlg, message, wParam, lParam,
                                          info->mask );
        }
        if ( !ret ) {
            if ( info->obj_id == DIALOG_OBJ ) {
