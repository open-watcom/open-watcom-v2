...
static void * MemUprCpy( char * dst, const char _WCUNALIGNED * src,
                         size_t length )
/*****************************************************************/
{
    char *                      c_dst;
    const char _WCUNALIGNED *   c_src;
...
static void * MemUprCpyUni( void * dst, const char _WCUNALIGNED * src,
                            size_t length )
/********************************************************************/
{
    uint_16 *                   c_dst;
...
/* the string list should be filled in when this is called */
{
...
#else
    cnt = str->StringListLen / sizeof( void * );
    //I don't think this should ever happen
    if( str->StringListLen % sizeof( void * ) ) cnt++;
    for( i=0; i < cnt; i++ ) {
    ...
    str->StringBlock = WRESALLOC( str->StringBlockSize );
...
//    void ** new_list;
    ...
    if (WResIsEmpty( dir )) {
        ...
        str->StringList = WRESALLOC( list_len * sizeof(void *) );
        ...
        #if 0
        /* shrink the list down to its actual size */
        new_list = WRESALLOC( list_len * sizeof(void *) );
        memcpy(new_list, str->StringList, list_len * sizeof(void*));
        WRESFREE(str->StringList);
        if (new_list != NULL || list_len == 0) {
...
static int genericCompare( char *name1, uint_16 len1,
                           char *name2, uint_16 len2, int use_unicode ) {
/**************************************************************************/
    int                 char_num;
    uint_16             ch1;
    uint_16             ch2;
...
static int compareStrings16( const void *vkey, const void *p2 )
/*************************************************************/
{
    StringItem16 **     item;
    const WResIDName *  key;

    item = (StringItem16 **) p2;
    key = vkey;
    return( genericCompare( key->Name, key->NumChars,
                            (*item)->Name, (*item)->NumChars, FALSE ) );
}

static int compareStrings32( const void *vkey, const void *p2 )
/*************************************************************/
{
    StringItem32 **     item;
    const WResIDName *  key;

    item = (StringItem32 **) p2;
    key = vkey;
    return( genericCompare( key->Name, key->NumChars,
                            (*item)->Name, (*item)->NumChars, TRUE ) );
}

int CompareStringItems32( StringItem32 *item1, StringItem32 *item2 ) {
/********************************************************************/
    uint_16     *ptr1;
    uint_16     *ptr2;
    uint_16      ch1;
...
/* number of bytes into the StringBlock at which to find the name. */
{
    ...
    if( str->UseUnicode ) {
        location = bsearch( (void *) name, str->StringList, str->StringListLen,
                        sizeof(void *), compareStrings32 );
    } else {
        location = bsearch( (void *) name, str->StringList, str->StringListLen,
                        sizeof(void *), compareStrings16 );
    }

    if( location == NULL ) {
        return( -1 );
    } else {
        return( *location - str->StringBlock );
    }
}
