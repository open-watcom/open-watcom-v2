...
/***********************************/
{
    ...
    STOSBInfo.cclass = FECALL_X86_NO_FLOAT_REG_RETURNS |
...
/******************************/
{
    ...
    if( AuxInfo.cclass & FECALL_X86_SPECIAL_RETURN )
        CurrInfo->returns = AuxInfo.returns;
    if( AuxInfo.cclass & FECALL_X86_SPECIAL_STRUCT_RETURN )
    ...
    if( !HW_CEqual( AuxInfo.save, HW_EMPTY ) ) {
        ...
        if( (AuxInfo.cclass & FECALL_X86_MODIFY_EXACT) == 0 && !CompFlags.save_restore_segregs ) {
...
/********************************/
{
     ...
     */
//    info->cclass |= FECALL_X86_GENERATE_STACK_FRAME;
...
/*****************************/
{
    ...
    for( ;; ) {
        if( !have.f_float && PragRecogId( "float" ) ) {
            have.f_float = true;
            AuxInfo.cclass |= FECALL_X86_NO_FLOAT_REG_RETURNS;
        } else if( !have.f_struct && PragRecogId( "struct" ) ) {
            have.f_struct = true;
            AuxInfo.cclass |= FECALL_X86_NO_STRUCT_REG_RETURNS;
        } else if( !have.f_allocs && PragRecogId( "routine" ) ) {
            have.f_allocs = true;
            AuxInfo.cclass |= FECALL_X86_ROUTINE_RETURN;
            ...
            have.f_list = true;
            AuxInfo.cclass |= FECALL_X86_SPECIAL_STRUCT_RETURN;
...
/***************************/
{
    ...
    have.f_list = false;
    have.f_struct = false;
    AuxInfo.cclass &= ~ FECALL_X86_NO_8087_RETURNS;
    ...
    for( ;; ) {
        if( !have.f_no8087 && PragRecogId( "no8087" ) ) {
            ...
            have.f_list = true;
            AuxInfo.cclass |= FECALL_X86_SPECIAL_RETURN;
    ...
    if( GetPragmaAuxAliasInfo() ) {
        ...
        for( ;; ) {
            if( !have.f_call && CurToken == T_EQUAL ) {
                ...
                AuxInfo.cclass |= FECALL_X86_FAR_CALL;
                have.f_call = true;
                ...
                AuxInfo.cclass |= FECALL_X86_LOAD_DS_ON_ENTRY;
                have.f_loadds = true;
                ...
                AuxInfo.cclass |= FECALL_X86_GENERATE_STACK_FRAME;
                have.f_frame = true;
            } else {
                break;
            }
