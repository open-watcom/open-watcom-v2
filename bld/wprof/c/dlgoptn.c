...
#define DLGOPTS_CTLS() \
    pick_p5(   BARGRAPHBOX, DLG_BOX,            "Bar Graph",                    C1+1, R0,   W-C1-2, 5 ) \
    pick_p4id( STRETCH,     DLG_CHECK,          "&Stretch the graph",           C1+2, R0+1, W-C1-4 ) \
    pick_p4id( ABS_BAR,     DLG_CHECK,          "Show &Absolute Bar Graphs",    C1+2, R0+2, W-C1-4 ) \
    pick_p4id( REL_BAR,     DLG_CHECK,          "Show &Relative Bar Graphs",    C1+2, R0+3, W-C1-4 ) \
    pick_p5(   SORTBYBOX,   DLG_BOX,            "Sort by",                      C1+1, R1,   W-C1-2, 4 ) \
    pick_p4id( SORT_COUNT,  DLG_RADIO_START,    "Sample &Count",                C1+2, R1+1, W-C1-4 ) \
    pick_p4id( SORT_NAME,   DLG_RADIO_END,      "&Name",                        C1+2, R1+2, W-C1-4 ) \
    pick_p5(   GATHERBOX,   DLG_BOX,            "Gather",                       C1+1, R2,   W-C1-2, 5 ) \
    pick_p4id( GATHER_TEXT, DLG_DYNSTRING,      "Cutoff Percentage",            C1+2, R2+1, W-C1-4 ) \
    pick_p4id( GATHER_CUT,  DLG_EDIT,           "",                             W-13, R2+1, 11 ) \
    pick_p4id( GATHER,      DLG_CHECK,          "Gather Small &Values",         C1+2, R2+3, W-C1-4 ) \
    pick_p4id( OK,          DLG_DEFBUTTON,      "OK",                           B1,   R3,   BW+1 ) \
    pick_p4id( DEFAULTS,    DLG_BUTTON,         "&Defaults",                    B2,   R3,   BW+1 ) \
    pick_p4id( CANCEL,      DLG_BUTTON,         "Cancel",                       B3,   R3,   BW+1 )
