...
extern dip_status hllTypeInfo( imp_image_handle *ii, imp_type_handle *ith,
...
static virt_mem TypeIndexVM( imp_image_handle *ii, unsigned idx )
{
    ...
    if( idx < CV_FIRST_USER_TYPE )
        return( 0 );
    cde = (cv_directory_entry *)hllFindDirEntry( ii, IMH_GBL, sstGlobalTypes );
    if( cde == NULL )
        return( 0 );
    p = VMBlock( ii,
        cde->lfo
...
dip_status hllTypeIndexFillIn( imp_image_handle *ii, unsigned idx, imp_type_handle *ith )
{
    ith->array_dim = 0;
    ith->idx = idx;
    if( idx < CV_FIRST_USER_TYPE ) {
    ...
    if( ith->handle == 0 ) return( DS_ERR | DS_FAIL );
                                ...
                                lf_all **pp )
{
    ...
    *out_ith = *in_ith;
    p = NULL;
    for( ;; ) {
        if( out_ith->handle == 0 ) goto done;
        p = VMBlock( ii, out_ith->handle, sizeof( *p ) );
        if( p == NULL ) return( DS_ERR | DS_FAIL );
        switch( p->common.code ) {
        case LF_MODIFIER:
            if( hllTypeIndexFillIn( ii, p->modifier.f.index, out_ith ) != DS_OK ) {
                return( DS_ERR | DS_FAIL );
            }
            break;
        default:
                            ...
                            cv_calls *call_type, unsigned *parms )
{
    ...
    ds = hllTypeIndexFillIn( ii, idx, &ith );
...
dip_status hllTypeMemberFuncInfo( imp_image_handle *ii, imp_type_handle *func_ith,
                                  ...
                                  unsigned long *adjustp )
{
    ...
    if( this_ith != NULL ) {
        ds = hllTypeIndexFillIn( ii, p->mfunction.f.thisptr, this_ith );
        if( ds != DS_OK ) return( ds );
    }
    if( class_ith != NULL ) {
        ds = hllTypeIndexFillIn( ii, class_idx, class_ith );
        p = VMBlock( ii, class_ith->handle, sizeof( *p ) );
        if( p->common.code == LF_MODIFIER ){ //NON standard need it to handle forward refs
            ds = hllTypeIndexFillIn( ii, p->modifier.f.index, class_ith );
        }
        if( ds != DS_OK ) return( ds );
    }
    return( DS_OK );
            ...
            unsigned_32 vfunc_off, location_context *lc, location_list *ll )
{
    ...
    ds = hllTypeIndexFillIn( ii, data.ptr_idx, &ith );
    if( ds != DS_OK ) return( ds );
                              ...
                              location_context *lc, location_list *ll )
{
    ...
    case LF_MEMBER:
        ...
        if( idx >= CV_FIRST_USER_TYPE ) {
            ...
            ds = hllTypeIndexFillIn( ii, idx, &base_ith );
            if( ds != DS_OK ) return( ds );
                              ...
                              imp_type_handle *ith )
{
    ...
    return( hllTypeIndexFillIn( ii, idx, ith ) );
}
...
walk_result hllTypeSymWalkList( imp_image_handle *ii, imp_type_handle *ith,
...
search_result hllTypeSearchNestedSym( imp_image_handle *ii, imp_type_handle *ith, lookup_item *li, void *d )
{
    lf_all              *p;
...
dip_status hllTypeInfo( imp_image_handle *ii, imp_type_handle *ith,
                        location_context *lc, dip_type_info *ti )
{
    imp_type_handle             real_ith;
    dip_status                  ds;
    ...
    case LF_DIMARRAY:
        ...
        ds = hllTypeArrayInfo( ii, ith, lc, &ai, NULL );
        if( ds != DS_OK )
            return( ds );
...
dip_status DIPIMPENTRY( TypeInfo )( imp_image_handle *ii, imp_type_handle *ith,
                                    location_context *lc, dip_type_info *ti )
{
    return( hllTypeInfo( ii, ith, lc, ti ) );
}

dip_status hllTypeBase( imp_image_handle *ii, imp_type_handle *ith, imp_type_handle *base_ith )
{
    dip_status                  ds;
    cv_primitive                prim;
    lf_all                      *p;
    ...
    switch( p->common.code ) {
    case LF_POINTER:
        return( hllTypeIndexFillIn( ii, p->pointer.f.type, base_ith ) );
    case LF_ARRAY:
        return( hllTypeIndexFillIn( ii, p->array.f.elemtype, base_ith ) );
    case LF_CLASS:
    case LF_STRUCTURE:
    case LF_UNION:
        return( DS_ERR|DS_BAD_PARM );
    case LF_ENUM:
        return( hllTypeIndexFillIn( ii, p->enum_.f.type, base_ith ) );
    case LF_PROCEDURE:
        return( hllTypeIndexFillIn( ii, p->procedure.f.rvtype, base_ith ) );
    case LF_MFUNCTION:
        return( hllTypeIndexFillIn( ii, p->mfunction.f.rvtype, base_ith ) );
    case LF_COBOL0:
    case LF_COBOL1:
    case LF_BARRAY:
    case LF_LABEL:
    ...
    case LF_DIMARRAY:
        ...
        ds = hllTypeIndexFillIn( ii, p->dimarray.f.diminfo, &dim_ith );
        if( ds != DS_OK )
            return( ds );
        ...
        return( hllTypeIndexFillIn( ii, save_idx, base_ith ) );
    case LF_PRECOMP:
    case LF_ENDPRECOMP:
    case LF_OEM:
    case LF_ARGLIST:
    ...
    case LF_FIELDLIST:
        return( DS_ERR | DS_BAD_PARM );
    case LF_BITFIELD:
        ds = hllTypeIndexFillIn( ii, p->bitfield.f.type, base_ith );
        if( ds != DS_OK )
            return( ds );
        return( hllTypeBase( ii, base_ith, base_ith ) );
    case LF_REFSYM:
        break;
    default:
        hllConfused();
                                    ...
                                    location_context *lc, location_list *ll )
{
    ...
    return( hllTypeBase( ii, ith, base_ith ) );
}
...
{
    /* unused parameters */ (void)ii; (void)vm; (void)size; (void)lc; (void)bound;
                        ...
                        array_info *ai, imp_type_handle *index_ith )
{
    ...
    ds = TypeReal( ii, array_ith, &real, NULL );
    if( ds != DS_OK )
        return( ds );
    ai->low_bound = 0;
    ai->num_elts = 0;
    ...
    case LF_DIMARRAY:
        ...
        if( ai->column_major ) {
            ...
            ds = hllTypeBase( ii, array_ith, &real );
            if( ds != DS_OK )
                return( ds );
            ds = hllTypeInfo( ii, &real, lc, &ti );
            if( ds != DS_OK )
    ...
    if( index_ith != NULL ) {
        return( hllTypeIndexFillIn( ii, idx, index_ith ) );
    }
    return( DS_OK );
}
...
dip_status DIPIMPENTRY( TypeArrayInfo )( imp_image_handle *ii,
                        ...
                        array_info *ai, imp_type_handle *index_ith )
{
    return( hllTypeArrayInfo( ii, array_ith, lc, ai, index_ith ) );
}
                ...
                imp_type_handle *proc_ith, imp_type_handle *parm_ith, unsigned n )
{
    ...
    if( n == 0 ) {
        return( hllTypeIndexFillIn( ii, rv_idx, parm_ith ) );
    }
    if( n <= parm_count ) {
        ds = hllTypeIndexFillIn( ii, al_idx, parm_ith );
        if( ds != DS_OK )
            return( ds );
        p = VMRecord( ii, parm_ith->handle - sizeof( unsigned_16 ), NULL, NULL );
        if( p == NULL )
            return( DS_ERR|DS_FAIL );
        ...
        return( hllTypeIndexFillIn( ii, arg_types[n-1], parm_ith ) );
    }
    return( DS_NO_PARM );
}
...
{
    /* unised parameters */ (void)ii; (void)ith; (void)lc; (void)a;
...
{
    /* unised parameters */ (void)ii; (void)ith;
