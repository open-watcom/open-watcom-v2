...
static bool             main_eof_seen = false;  // Ctrl-Z seen in main asm file
...
static bool get_asmline( char *ptr, unsigned max, FILE *fp, bool *eof_seen )
/**************************************************************************/
{
    int         quote = '\0';
    bool        skip;
    ...
    for( ;; ) {
        ...
        case 0x1A:
             ...
             * files), so set the eof flag to stop all subsequent reads.
             */
            if( quote == '\0' ) {
                *ptr = '\0';
                *eof_seen = true;
...
/************************************/
{
    ...
    while( file_stack != NULL ) {
        ...
        if( inputfile->is_a_file ) {
            if( !inputfile->eof_seen
              && get_asmline( string, MAX_LINE_LEN, inputfile->u.file, &inputfile->eof_seen ) ) {
                LineNumber++;
                return( string );
            }
            /*
 ...
 */
{
    ...
    if( line != NULL )
        return( line );
    if( main_eof_seen
      || !get_asmline( string, MAX_LINE_LEN, AsmFiles.file[ASM], &main_eof_seen ) ) {
        return( NULL );
    }
    LineNumber++;
    return( string );
