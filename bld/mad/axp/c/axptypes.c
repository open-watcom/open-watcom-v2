...
static mad_status DoConvert( const mad_type_info *in_mti, const void *in_d, const mad_type_info *out_mti, void *out_d )
{
    ...
    case AXPT_F_FLOAT:
        temp[0] = p1[1];
        temp[1] = p1[0];
        return( MCTypeConvert( TypeArray[AXPT_RF_FLOAT].u.info, temp, out_mti, out_d, 0 ) );
    case AXPT_G_FLOAT:
        temp[0] = p1[3];
        temp[1] = p1[2];
        temp[2] = p1[1];
        temp[3] = p1[0];
        return( MCTypeConvert( TypeArray[AXPT_RG_FLOAT].u.info, temp, out_mti, out_d, 0 ) );
    case AXPT_D_FLOAT:
        temp[0] = p1[3];
        temp[1] = p1[2];
        temp[2] = p1[1];
        temp[3] = p1[0];
        return( MCTypeConvert( TypeArray[AXPT_RD_FLOAT].u.info, temp, out_mti, out_d, 0 ) );
    }
    p2 = out_d;
    switch( out_mti->b.handler_code ) {
    case AXPT_F_FLOAT:
        ms =  MCTypeConvert( in_mti, in_d, TypeArray[AXPT_RF_FLOAT].u.info, temp, 0 );
        if( ms != MS_OK ) return( ms );
        p2[1] = temp[0];
        p2[0] = temp[1];
        break;
    case AXPT_G_FLOAT:
        ms =  MCTypeConvert( in_mti, in_d, TypeArray[AXPT_RG_FLOAT].u.info, temp, 0 );
        if( ms != MS_OK ) return( ms );
        p2[3] = temp[0];
        p2[2] = temp[1];
        p2[1] = temp[2];
        ...
        break;
    case AXPT_D_FLOAT:
        ms =  MCTypeConvert( in_mti, in_d, TypeArray[AXPT_RD_FLOAT].u.info, temp, 0 );
        if( ms != MS_OK ) return( ms );
        p2[3] = temp[0];
        p2[2] = temp[1];
        p2[1] = temp[2];
...
mad_status MADIMPENTRY( TypeToString )( mad_radix radix, const mad_type_info *mti, const void *data, char *buff, size_t *buff_size_p )
{
    ...
    case AXPT_F_FLOAT:
        new_mti = TypeArray[AXPT_FLOAT].u.info;
        break;
    case AXPT_G_FLOAT:
    case AXPT_D_FLOAT:
        new_mti = TypeArray[AXPT_DOUBLE].u.info;
        break;
    default:
        return( MS_UNSUPPORTED );
    }
