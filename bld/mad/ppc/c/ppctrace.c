...
{
    if( U64Low( mr->ppc.u1.sp ) < watch_stack.mach.offset ) {
        return( MS_OK );
    }
    return( MS_FAIL );
}
...
mad_trace_how MADIMPENTRY( TraceOne )( mad_trace_data *td, mad_disasm_data *dd, mad_trace_kind tk, mad_registers const *mr, address *brk )
{
    ...
    ra = td->ra;
    td->ra = U64Low( mr->ppc.iar ) + sizeof( unsigned_32 );
    switch( tk ) {
    case MTRK_OUT:
...
mad_status MADIMPENTRY( UnexpectedBreak )( mad_registers *mr, char *buff, size_t *buff_size_p )
{
    ...
    *buff_size_p = 0;
    ...
    if( data.brk != BRKPOINT )
        return( MS_FAIL );
    U64Low( mr->ppc.iar ) += sizeof( unsigned_32 );
    if( data.br != JMP_SHORT )
        return( MS_OK );
    if( memcmp( data.name, "WVIDEO\0\0", 8 ) != 0 )
        return( MS_OK );
    a.mach.offset = U64Low( mr->ppc.r3 );
    len = 0;
