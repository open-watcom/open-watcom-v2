...
/******************************************************/
{
    ...
    if( strcmp( NameStr( name ), "dllimport" ) == 0 ) {
      ...
      || strcmp( NameStr( name ), "_Noreturn" ) == 0 ) {
...
/********************************/
{
    ...
    */
    ...
    if( idexpr ) {
        ...
        if ( is_qualified ) {
            if( !AnalyseClQualRes( &expr, &result ) )
                goto EXIT;
            if( !AnalyseSymbolAccess( expr, expr, NULL, &diagMemb ) )
                goto EXIT;
...
/*************************************/
{
    TypeStripTdMod( type );
    for(;;) {
        if( ! TypeIdMasked( type, MASK_TYPE_TDEF_ARR ) )
            break;
        type = type->of;
    }
    return( type );
    ...
    for(;;) {
        ...
        if( (*u_top)->op == PT_INT_CONSTANT ) {
            if( (*b_top)->op == PT_INT_CONSTANT ) {
                if( U64isEq( (*u_top)->u.int64_constant, (*b_top)->u.int64_constant ) ) {
                    PTreeFree( *b_top );
                    PTreeFree( *u_top );
                    continue;
                }
