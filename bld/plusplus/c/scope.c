...
static SYMBOL injectOpDel( NAME name )
{
    ...
    sym = SymCreateFileScope( TypeVoidFunOfPtrVoid(), SYMC_EXTERN, SYMF_NONE, name );
    LinkageSet( sym, "C++" );
    return( sym );
}
...
static void injectOpNew( TYPE fn_type, NAME name )
{
    ...
    sym = SymCreateFileScope( fn_type, SYMC_EXTERN, SYMF_NONE, name );
    LinkageSet( sym, "C++" );
}
...
static void injectChipBug( void )
{
    ...
    sym = SymCreateFileScope( type, SYMC_EXTERN, SYMF_NONE, name );
    LinkageSet( sym, "C" );
    ChipBugSym = sym;
}
...
/*********************************/
{
    ...
    if( CompFlags.pch_debug_info_write ) {
        ...
        type = GetBasicType( TYP_CHAR );
        sym = SymCreateFileScope( type, SYMC_PUBLIC, SYMF_NONE, name );
        PCHDebugSym = sym;
...
{
    if( sym->flags & (SYMF_REFERENCED | SYMF_NO_REF_WARN) ) {
        return;
    }
    if( SymIsInline( sym ) ) {
        return;
    ...
    if( SymIsFunction( sym ) && (sym->flags & SYMF_ADDR_TAKEN) ) {
        return;
    }
...
/***********************************************/
{
    ...
    RingIterBeg( ScopeFriends( scope ), a_friend ) {
        if( FriendIsSymbol( a_friend ) ) {
            ...
            if( friendly_sym->id == SYMC_CLASS_TEMPLATE ) {
        ...
        if( FriendIsType( a_friend ) ) {
            ...
            if( sym->id == SYMC_CLASS_TEMPLATE ) {
                ...
                if( SymIsFunction( sym ) ) {
                    ...
                    if( ( curr_sym != NULL )
                     && ( curr_sym->flags & SYMF_TEMPLATE_FN )
...
static inherit_flag makePerm( symbol_flags flags )
{
    if( flags & SYMF_PRIVATE ) {
        return( IN_PRIVATE );
    }
    if( flags & SYMF_PROTECTED ) {
        return( IN_PROTECTED );
    }
    return( IN_PUBLIC );
}
...
static walk_status memberSearch( BASE_STACK *top, void *parm )
{
    ...
    if( data->check_special ) {
        ...
        if( syms != NULL && syms->id == SYMC_ACCESS ) {
            /* this member was an access control; record and keep going */
            top->access_perm = makePerm( syms->flags );
...
/****************************************************/
{
    ...
    sym->flags |= SYMF_ADDR_TAKEN | SYMF_CG_ADDR_TAKEN;
...
/****************************************************/
{
    ...
    if( result == NULL ) {
        ...
        sym = SymCreateAtLocn( fn_type, SYMC_EXTERN, SYMF_NONE, name, GetFileScope(), NULL );
        LinkageSet( sym, "C" );
        return;
    }
    aux_sym = NULL;
...
static void saveSymbol( void *e, carve_walk_base *d )
{
    ...
    case SYMC_ACCESS:
        ...
        break;
    default:
        if( s->flags & (SYMF_ADDR_THUNK | SYMF_TEMPLATE_FN | SYMF_ALIAS) ) {
            save_u_sym = s->u.sym;
            s->u.sym = SymbolGetIndex( save_u_sym );
        } else if( s->flags & SYMF_CONSTANT_INT64 ) {
            save_con = s->u.pval;
            s->u.pval = ConstantPoolGetIndex( save_con );
        } else if( (s->flags & SYMF_ANONYMOUS) && ( s->id != SYMC_MEMBER ) ) {
            save_u_sym = s->u.sym;
            s->u.sym = SymbolGetIndex( save_u_sym );
        }
    }
    ...
    case SYMC_ACCESS:
        ...
        break;
    default:
        DbgVerify( SymIsThunk( s ) == ( (s->flags & SYMF_ADDR_THUNK) != 0 ), "SYMF_ADDR_THUNK not set properly" );
        if( s->flags & (SYMF_ADDR_THUNK | SYMF_TEMPLATE_FN | SYMF_ALIAS) ) {
            s->u.sym = save_u_sym;
        } else if( s->flags & SYMF_CONSTANT_INT64 ) {
            s->u.pval = save_con;
...
static void readSymbols( void )
{
    ...
    for( ; (i = PCHReadCVIndex()) != CARVE_NULL_INDEX; ) {
        ...
        case SYMC_ACCESS:
            ...
            break;
        default:
            if( sym->flags & (SYMF_ADDR_THUNK | SYMF_TEMPLATE_FN | SYMF_ALIAS) ) {
                sym->u.sym = SymbolMapIndex( sym->u.sym );
            } else if( sym->flags & SYMF_CONSTANT_INT64 ) {
                sym->u.pval = ConstantPoolMapIndex( sym->u.pval );
            } else if( (sym->flags & SYMF_ANONYMOUS) && ( sym->id != SYMC_MEMBER ) ) {
                sym->u.sym = SymbolMapIndex( sym->u.sym );
            }
        }
    }
