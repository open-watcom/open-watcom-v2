...
static void expandMacroToken( MACRO_TOKEN *m )
{
    switch( m->token ) {
    case T_CONSTANT:
    case T_PPNUMBER:
    ...
    MEPTR mentry )              // - macro entry
{
    ...
    case MACRO_FUNC:
        ...
        if( FUNCTION_not_expandable ) {
            WriteBufferString( SpcMacros[mentry->u.special_macro].name );
            return( T_ID );
        }
        ...
        if( sym != NULL ) {
            ...
            WriteBufferString( VbufString( &buff ) );
            VbufFree( &buff );
        } else {
            WriteBufferString( "?" );
        }
...
static exp_state isExpandable( MEPTR mentry, MACRO_TOKEN *mtok, bool macro_parm )
{
    ...
    if( MacroIsSpecial( mentry ) ) {  /* if special macro */
        if( ( mentry->u.special_macro == MACRO_FUNCTION ) || ( mentry->u.special_macro == MACRO_FUNC ) ) {
            if( FUNCTION_not_expandable ) {
...
static MACRO_TOKEN *glue2Tokens( MACRO_TOKEN *first, MACRO_TOKEN *second )
{
    ...
    TokenLen = INPOUT_SHIFT;
    if( first != NULL ) {
        expandMacroToken( first );
    }
    if( second != NULL ) {
        expandMacroToken( second );
    }
    WriteBufferNullChar();
...
static MACRO_TOKEN *substituteParms( MACRO_TOKEN *head, MACRO_ARG *macro_parms )
{
    ...
    for( mtok = head; mtok != NULL; mtok = mtok->next ) {
        ...
        if( mtok->token == T_MACRO_PARM || mtok->token == T_MACRO_VAR_PARM ) {
            ...
            dummy_list = NULL;
            parm_idx = GET_MPARM( mtok->data );
            p = macro_parms[parm_idx].arg;
