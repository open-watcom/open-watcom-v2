...
static carve_t                  carve_ppoperator_stack; // carver: operator stack entries
...
/**************************************************************/
{
    ...
    stack_entry = CarveAlloc( carve_ppoperator_stack );
...
/****************************************************/
{
    ...
    if( stack_entry != NULL ) {
        ...
        CarveFree( carve_ppoperator_stack, stack_entry );
        return( true );
    }
    return( false );
}
...
/**************************/
{
    ...
    case T_ID:
        ...
        if( IS_PPOPERATOR_DEFINED( Buffer ) ) {
            ...
            if( CurToken == T_LEFT_PAREN ) {
                ...
                PPControl = old_ppctl;
                Set64ValU32PP( p, MacroDependsDefined() );
                NextToken();    // no need to error check or advance Pos
    ...
    case T_FALSE:
    case T_TRUE:
        Set64ValU32PP( p, CurToken == T_TRUE );
 ...
 */
{
    ...
    Set64ValZeroPP( *p );  // default value 0
    ...
    INITFINI* defn )            // - definition
{
    ...
    carve_ppoperator_stack = CarveCreate( sizeof( PPEXPN_OPERATOR_STACK ), 16 );
    HeadOperator = NULL;
}
    ...
    INITFINI* defn ) // - definition
{
    ...
    CarveDestroy( carve_ppoperator_stack );
    HeadOperator = NULL;
}
