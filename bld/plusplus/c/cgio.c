    ...
    CGINTER *ins )              // - instruction
{
    ...
    if( ICOpTypes[ins->opcode.id] == ICOT_NUL ) {
...
static void saveCGFILE( void *e, carve_walk_base *d )
{
    ...
    if( file->u.s.done ) {
        ...
        stop_cursor->opcode.id = IC_NO_OP;
        DbgAssert( stop_cursor->opcode.id != IC_NEXT );
        for(;;) {
            if( h == stop_buffer && cursor == stop_cursor )
                break;
            opcode = cursor->opcode.id;
            // might be the destination of a CgioZap
            zap_ref.opcode.id = IC_NO_OP;
            switch( opcode ) {
            case IC_BLOCK_OPEN:
            case IC_BLOCK_DEAD:
        ...
        terminator.opcode.id = IC_PCH_STOP;
