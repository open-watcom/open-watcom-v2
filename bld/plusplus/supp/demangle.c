...
#pragma message( "this will be obsolete soon; use $(lib_misc_dir)\c\demangle.c" )
...
#else
#define _output1( _a         )
#define _output2( _a, _b     )
#define _output3( _a, _b, _c )
#endif
...
static void emitStr( output_desc *data, char _WCI86FAR *p )
{
    if( p ) {
        while( *p ) {
            emitChar( data, *p );
...
static void demangleEmit( void **cookie, dm_pts dp, int value, char *ptr )
{
...
static int name( output_desc *data, state_desc *state )
{
    ...
    if( isdigit( c ) ) {
        ...
        if( data->ctdt_pending ) {
            ...
            _output3( DM_IDENTIFIER, replicate[c].len, replicate[c].ptr );
            data->ctdt_pending = FALSE;
        }
        if( data->scope_name && data->scope_index == 0 ) {
            data->scope_name = FALSE;
            data->scope_ptr = replicate[c].ptr;
            data->scope_len = replicate[c].len;
        }
        _output2( DM_SET_INDEX, state->prefix );
        _output3( DM_IDENTIFIER, replicate[c].len, replicate[c].ptr );
        _output1( DM_EMIT_SPACE );
...
static size_t demangle_recursive( char *input, char *output, size_t size )
{
...
int __is_mangled( char *name, size_t len )
{
    int offset;
    len = len;
...
static int checkInternal( char *n )
{
...
int __is_mangled_internal( char *name, size_t len )
{
    unsigned offset;
    len = len;
...
int __unmangled_name(                           // FIND UNMANGLED BASE NAME
    char *name,                                 // - mangled C++ name
    size_t len,                                 // - length of mangled name
    char **base,                                // - location of base name
...
void testEmit( void **cookie, dm_pts dp, int value, char *ptr )
{
...
void main( int argc )
{
    ...
    for( trunc_len = TRUNC_BUFFER ; trunc_len > 0 ; trunc_len-- ) {
        ...
        for( i = 0; typeTestVector[i] != NULL; i += 2 ) {
            len = __demangle_t( typeTestVector[i], 0, NULL, 0 );
            if( len == 0 ) {
                ++errors;
                printf( "ERROR: returned length is zero\n" );
            }
            buff[0] = buff[trunc_len+1] = GUARD_CHAR;
    ...
    for( i = 0 ; testVector[i].mangle != NULL ; ++i ) {
        if( __unmangled_name( testVector[i].mangle, 0, &p, &len ) ) {
            char buff[BUF_SIZE];
            memcpy( buff, p, len );
            buff[len] = 0;
            if( argc > 1 ) {
    ...
    for( i = 0 ; testVector[i].mangle != NULL ; ++i ) {
        ...
        free( p );
        __unmangled_name( testVector[i].mangle, 0, &p, &len );
        if( strncmp( p, testVector[i].unmangled, len ) ) {
            char buff[BUF_SIZE];
            memcpy( buff, p, len );
            buff[len] = 0;
