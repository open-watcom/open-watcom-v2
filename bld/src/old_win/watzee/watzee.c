...
long _EXPORT FAR PASCAL    WndProc( HWND, WORD, WORD, LONG );
...
/*******************************************************/
{
    ...
    if( !previnstance ) {
        ...
        wndclass.cbWndExtra     = 6 * sizeof( HBITMAP );
...
long _EXPORT FAR PASCAL WndProc( HWND hwnd, WORD message, WORD wparam,
                                  LONG lparam )
/*********************************************************************/
{
...
//      SetFocus( hwnd );
    ...
    case WM_DESTROY :
        ...
        for( i = 0; i < 6; i++ ) {
            DeleteObject( GetWindowWord( hwnd, i*2 ) );
        }
        PostQuitMessage( 0 );
        return( 0 );
    }
...
/***********************************************/
{
    ...
    die_checked = 0;
    GetObject( GetWindowWord( hwnd, 0 ), sizeof( BITMAP ), (LPSTR) &bm );
