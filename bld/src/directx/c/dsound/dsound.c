...
int init_dsound(void)
{
	...
	err = IDirectSound8_SetCooperativeLevel(dsound, game_window, DSSCL_EXCLUSIVE);
	if (err != DS_OK)
		return 1;
	...
	err = IDirectSound8_CreateSoundBuffer(dsound, &dsbdesc, &dsprimarybuffer, NULL);
	if (err != DS_OK)
		return 1;
	...
	err = IDirectSoundBuffer_SetFormat(dsprimarybuffer, &wfx);
	if (err != DS_OK)
		return 1;

	//force dsound to ALWAYS keep DMA active
	err = IDirectSoundBuffer_Play(dsprimarybuffer, 0, 0, DSBPLAY_LOOPING);
	if (err != DS_OK)
		return 1;
...
void shutdown_dsound()
{
	if (sfxbuffer8)
	{
		IDirectSoundBuffer8_Stop(sfxbuffer8);
		IDirectSoundBuffer8_Release(sfxbuffer8);
	}
	if (dsprimarybuffer)
	{
		IDirectSoundBuffer_Stop(dsprimarybuffer);
		IDirectSoundBuffer_Release(dsprimarybuffer);
	}
	if (dsound) IDirectSound8_Release(dsound);
...
int load_wav(char *filename)
{
	...
	if (*riff == MRIFFID('R','I','F','F'))
	{
		...
		err = IDirectSound8_CreateSoundBuffer(dsound, &dsbufferdesc, &sfxbuffer, NULL);
		if (err != DS_OK)
		{
			free(sfx);
			return 1;
		...
		err = IDirectSoundBuffer_QueryInterface(sfxbuffer, &IID_IDirectSoundBuffer8, (void **)&sfxbuffer8);
		if (sfxbuffer8 == NULL)
		{
			free(sfx);
			return 1;
		...
		err = IDirectSoundBuffer_Release(sfxbuffer);

		// Lock the buffer
		err = IDirectSoundBuffer8_Lock(sfxbuffer8, 0, 0, (void **)&data1, &size1, (void **)&data2, &size2, DSBLOCK_ENTIREBUFFER);
		if (err != DS_OK)
		{
			free(sfx);
			return 1;
		...
		err = IDirectSoundBuffer8_Unlock(sfxbuffer8, data1, size1, data2, size2);

		// Set the sfx volume
		err = IDirectSoundBuffer8_SetVolume(sfxbuffer8, vol_to_db(127));

		// Play the sound
		err = IDirectSoundBuffer8_Play(sfxbuffer8,0,0,0);
		if (err != DS_OK)
		{
			free(sfx);
			return 1;
