...
dis_handler_return SPARCSetHi( dis_handle *h, void *d, dis_dec_ins *ins )
{
    ...
    U64Low( ins->op[0].value ) = DisSEX( code.sethi.imm22, 21 );
...
dis_handler_return SPARCBranch( dis_handle *h, void *d, dis_dec_ins *ins )
{
    ...
    U64Low( ins->op[0].value ) = ( DisSEX( code.branch.disp22, 21 ) ) * sizeof( ins->opcode );
...
dis_handler_return SPARCCall( dis_handle *h, void *d, dis_dec_ins *ins )
{
    ...
    //U64Low( ins->op[0].value ) = ( DisSEX( code.call.disp, 29 ) + 1) * sizeof( ins->opcode );
    U64Low( ins->op[0].value ) = ( DisSEX( code.call.disp, 29 ) ) * sizeof( ins->opcode );
    ins->num_ops     = 1;
    return( DHR_DONE );
}
...
dis_handler_return SPARCOp3( dis_handle *h, void *d, dis_dec_ins *ins )
{
    ...
    if( code.op3.imm != 0 ) {
        ...
        U64Low( ins->op[ 1 ].value ) = DisSEX( code.op3imm.simm13, 12 );
    } else {
        ins->op[ 1 ].type = DO_REG;
...
static void doSparcMem( dis_handle *h, void *d, dis_operand *op, sparc_ins code )
{
    ...
    U64Low( op->value ) = 0;
    if( code.op3.imm != 0 ) {
        U64Low( op->value ) = DisSEX( code.op3imm.simm13, 12 );
    } else {
