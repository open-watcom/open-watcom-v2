...
PerlString w;
PerlStringList l;
PerlStringList ttfonts;
Assoc<PerlString> repfnts("", "");  // saves font replacement names
...
//      cout << "line " << ln << ": <" << s << ">" << endl;
        if(s.m("\\[fnt\\]")){
            ...
            ln++;
            sl= s.split(" \t"); // this extracts the font name
            fnt= sl.join(" ");  // this puts the words back together
