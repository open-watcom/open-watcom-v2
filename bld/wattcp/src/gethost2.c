...
static int reverse_lookup (question_t *q, char *name, DWORD nameserver)
{
    ...
    int        ready = 0;
    udp_Socket udp_sk;
    sock_type  *dom_sk = (sock_type *)&udp_sk;

    if (!nameserver || dns_timeout == 0)  /* no nameserver, give up */
        return (0);

    udp_open (&dom_sk->udp, 997, nameserver, 53, NULL);

    for (i = 2; i < 17 && !_resolve_exit; i *= 2) {
        sock_write (dom_sk, (BYTE*)q, sizeof(*q));
        ip_timer_init (dom_sk, i);
        do {
            kbhit();
            tcp_tick (dom_sk);
