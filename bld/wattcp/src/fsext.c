...
int _fsext_demux (__FSEXT_Fnumber func, int *rv, va_list _args)
{
  ...
  Socket *_socket = __FSEXT_get_data (fd); /* same as _socklist_find(fd) */

  SOCK_DEBUGF ((NULL,
     "\n_fsext_demux: fd %d%c, func %d = \"%s\", cmd %08lX, arg %08lX",
     fd, _socket ? 's' : 'f', func, lookup (func, fs_func, DIM(fs_func)),
     (DWORD)cmd, (DWORD)arg));
  ...
  switch (func)
  {
    ...
    case __FSEXT_ready:   /* ready (fd) */
         ...
         tcp_tick (NULL);
         *rv = 0;
         if (_sock_read_select(_socket) > 0)
            *rv |= __FSEXT_ready_read;

         if (_sock_write_select(_socket) > 0)
            *rv |= __FSEXT_ready_write;

         if (_sock_exc_select(_socket) > 0)
    ...
    case __FSEXT_dup2:    /* dup2 (oldfd,newfd) */
         {
           int type  = _socket->so_type;  /* don't use 'socket' after close() */
           int proto = _socket->so_proto;
