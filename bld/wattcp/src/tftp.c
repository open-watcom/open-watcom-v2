 ...
 * Local variables
 */
static struct tftphdr *inbuf = NULL;    /* TFTP input buffer  */
static struct tftphdr *outbuf = NULL;   /* TFTP output buffer */
static sock_type      *tftp_sk = NULL;  /* Socket for UDP recv/xmit */
...
static void send_req (char request, const char *fname)
{
    ...
    for ( ; *fname != '\0' && len > 0 && fnamlen > 0; len--, fnamlen--)
        *cp++ = *fname++;
    *cp++ = '\0';

    for (mode = tftp_xfer_mode; *mode != '\0'; )
        *cp++ = *mode++;
    *cp++ = '\0';
    ...
    sock_fastwrite (tftp_sk, (BYTE*)outbuf, len);
}
...
static void send_ack (int block)
{
    ...
    sock_fastwrite (tftp_sk, (BYTE*)&ack, TFTP_HEADSIZE);
}
 ...
 */
static int udp_callback (sock_type *sk, int icmp_type)
{
    if (sk == tftp_sk && sk->u.ip_type == UDP_PROTO && icmp_type == ICMP_UNREACH) {
        /* In lack of a better way, pretend we got a FIN.
         * This causes sock_wait_input() below to break it's loop.
         */
        sk->udp.locflags |= LF_GOT_FIN;
        sk->udp.err_msg = icmp_type_str [ICMP_UNREACH];
    }
    return (1);
}
...
static int recv_packet (int block)
{
     ...
     */
    if (block == 1) {
        tftp_sk->udp.sol_callb = udp_callback;
    } else {
        tftp_sk->udp.sol_callb = NULL;
    }
    /* Read packet with timeout
     */
    sock_wait_input (tftp_sk, tftp_timeout, NULL, &status);

    len = sock_fastread (tftp_sk, (BYTE*)inbuf, TFTP_HEADSIZE+SEGSIZE);
...
static int tftp_open (DWORD server, const char *fname)
{
    ...
    for (retry = 0; retry < tftp_retry; retry++) {
        ...
        if (tftp_lport >= TFTP_PORT_LOW) {
            ...
            our_tid = Random (TFTP_PORT_LOW, USHRT_MAX);
        }

        /* Try to open a TFTP connection to the server
         */
        if (!udp_open(&tftp_sk->udp, our_tid, server, port, NULL)) {
