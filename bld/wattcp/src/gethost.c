...
struct hostent * gethostent (void)
{
    ...
    if (alias != NULL && *alias != '#' && *alias != ';') {
        ...
        do {
            if (*alias == '#' || *alias == ';')
                break;
            if (alist == NULL || (i == MAX_HOST_ALIASES) ||
...
struct hostent *gethostbyname (const char *name)
{
    ...
    for (h = host0; h != NULL; h = h->h_next) {
        char **alias;

        if (h->h_name != NULL && !stricmp(h->h_name, name)) {
        ...
        for (alias = h->h_aliases; alias != NULL && *alias != NULL; alias++) {
...
expired:
    ...
    if (ip) {              /* successfully resolved */
        ...
        ret.h_address = ip;
        ret.h_name    = (char*) name;  /* !!to-do: should be canonical name */
        ret.h_aliases = h != NULL ? h->h_aliases : NULL;
        return FillHostEnt (&ret);
    }
 ...
 */
static __inline int getresult (sock_type *sk, char *name)
{
    answer_t a;
    struct   rrpart *rr;
    char    *c;
    int      len = sock_fastread (sk, (BYTE*)&a, sizeof(a));
...
static int reverse_lookup (question_t *q, char *name, DWORD nameserver)
{
    ...
    int        quit  = 0;
    udp_Socket dom_sk;
    sock_type *sk = (sock_type *)&dom_sk;

    if (!nameserver ||         /* no nameserver, give up */
        dns_timeout == 0)
        return (0);

    if (!udp_open(&sk->udp, 997, nameserver, 53, NULL))
        return (0);

    for (sec = 2; sec < dns_timeout-1 && !quit && !_resolve_exit; sec *= 2) {
        sock_write (sk, (BYTE*)q, sizeof(*q));
        ip_timer_init (sk, sec);
        do {
            kbhit();
            tcp_tick (sk);
            ...
            if (ip_timer_expired(sk) || chk_timeout(resolve_timeout)) {
                ready = 0;
                resolve_timeout = 1;
                break;  /* retry */
            }
