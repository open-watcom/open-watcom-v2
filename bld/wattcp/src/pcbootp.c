...
int _dobootp (void)
{
    ...
    int             status;
    udp_Socket      udp_sk;
    sock_type       *bootp_sk = (sock_type*)&udp_sk;
    ...
    if (!udp_open(&bootp_sk->udp, IPPORT_BOOTPC, _bootphost, IPPORT_BOOTPS, NULL)) {
    ...
    *(DWORD*) &bootp_out.bp_vend = intel (VM_RFC1048);  /* Magic Number */
    ...
    for ( ;; ) {
        ...
        sock_fastwrite (bootp_sk, (BYTE*)&bootp_out, sizeof(bootp_out));
        ...
        while (!chk_timeout(sendtimeout)) {
            ...
            kbhit();
            sock_tick (bootp_sk, &status);

            if (!sock_dataready(bootp_sk))
                continue;

            /* got a response, lets consider it
             */
            len = sock_fastread (bootp_sk, (BYTE*)&bootp_in, sizeof(bootp_in));
