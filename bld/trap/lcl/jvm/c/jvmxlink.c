...
//          strcpy( command, "dvw.exe " );
        ...
        if( !CreateProcess( NULL, (void*)command, NULL, NULL, TRUE,
                        CREATE_NEW_CONSOLE,
                        NULL, NULL, (void*)&sinfo, (void*)&pinfo ) ) {
            return( "unable to start " JAVA_NAME );
        }

        MemHdlLink = CreateFileMapping( (HANDLE)0xFFFFFFFF, NULL, PAGE_READWRITE,
                                    0, sizeof(HANDLE), REMOTE_LINK_MEM_NAME );
        if( MemHdlLink == NULL ) {
        ...
        *UniquePid = pinfo.dwProcessId;
        ...
        MemHdl2 = CreateFileMapping( (HANDLE)0xFFFFFFFF, NULL, PAGE_READWRITE,
                                    0, 1, SHARED_MEM_NAME_2  );
        SharedMem = MapViewOfFile( MemHdl, FILE_MAP_WRITE, 0, 0, 0 );
        Terminated = MapViewOfFile( MemHdl2, FILE_MAP_WRITE, 0, 0, 0 );
        if( MemHdl2 == NULL || MemHdl == NULL || SemReadUp == NULL ||
            SemReadDone == NULL || SemWritten == NULL ||
