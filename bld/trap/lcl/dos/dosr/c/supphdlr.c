...
static bool     Supporting8ByteBreakpoints = false;
static bool     SupportingExactBreakpoints = false;
...
trap_retval TRAP_CAPABILITIES( get_8b_bp )( void )
{
    ...
    ret = GetOutPtr( 0 );
    ret->err = 0;
    ret->status = true;         /* This signals we support 8 byte breakpoints */
    return( sizeof( *ret ) );
}
...
trap_retval TRAP_CAPABILITIES( get_exact_bp )( void )
{
    ...
    ret = GetOutPtr( 0 );
    ret->err = 0;
    ret->status = false;        /* This signals we not support exact breakpoints */
    return( sizeof( *ret ) );
}
