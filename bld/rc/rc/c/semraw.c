...
/******************************************/
{
    ...
    if( item.IsString ) {
        error = ResWriteStringLen( item.Item.String, item.StrLen, item.LongItem, false, CurrResFile.fp );
        if( !error ) {
            if( item.WriteNull ) {
                if( item.LongItem ) {
...
/**********************************************************************/
{
    ...
    if( !error ) {
        ...
        if( pos == -1L ) {
            ...
            ret = CopyDataUntilEOF( pos, fp, CurrResFile.fp, BUFFER_SIZE, &err_code );
            if( ret != RS_OK ) {
