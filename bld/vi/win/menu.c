...
static menu *addMenuToMenu( menu *m, char *name, char *help )
{
    ...
    new->item_tail = NULL;
    m->num_items += 1;
    AddLLItemAtEnd( &m->item_head, &m->item_tail, new );
    return( new );
...
static int freeItem( menu *m, int offset )
{
    ...
    if( citem != NULL ) {
        ...
        m->num_items -= 1;
        DeleteLLItem( &m->item_head, &m->item_tail, citem );
        MemFree( citem );
        return( TRUE );
    }
    return( FALSE );
...
static void freeMenu( menu *parent, menu *m )
{
    ...
    parent->num_items -= 1;
    DeleteLLItem( &parent->item_head, &parent->item_tail, m );
    MemFree( m );
    ...
    if( Root ) {
        if( rootMenu->menu_handle != NULL ) {
            for( m = (menu *)rootMenu->item_head; m != NULL; ) {
                ...
                DeleteLLItem( &rootMenu->item_head, &rootMenu->item_tail, m );
                MemFree( m );
                m = next;
            }
            // DestroyMenu( rootMenu->menu_handle );
...
static void purgeOldMenuBottom( menu *cmenu )
{
    ...
    if( cmenu->orig_num_items > 0 ) {
        ...
        while( cnt < cmenu->num_items ) {
            ...
            DeleteLLItem( &cmenu->item_head, &cmenu->item_tail, citem );
            MemFree( citem );
            citem = nitem;
            cnt++;
        }
