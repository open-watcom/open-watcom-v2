/**
 * @name Fast unbounded write no barrier
 * @description Detects unbounded buffer writes that may receive data from CodeQL flow sources.
 *              This variant avoids path reconstruction and guard/barrier analysis.
 * @kind problem
...
/*
 * An unbounded write is a buffer write without an explicit size limit
 * where CodeQL also cannot deduce an upper bound for the amount copied.
 */
...
/**
 * Reuses the standard CodeQL flow source classification.
 */
...
/**
 * Sink definition copied from the original UnboundedWrite query,
 * but without path reconstruction.
 */
...
/**
 * Taint-tracking configuration without custom guard/barrier analysis.
 */
module Config implements DataFlow::ConfigSig {
   ...
   * once the analysis reaches the unbounded write source buffer itself,
