...
shiftloop:
        ...
        push    ds
        pop     es
        mov     [edi].Real_EAX,1900h
        mov     Real_SS[edi],0
        mov     Real_SP[edi],0
        mov     bl,21h
        call    d[fRawSimulateINT]
        mov     al,BYTE PTR [edi].Real_EAX
        pop     edi
...
medtransloop:
        ...
        push    ds
        pop     es
        mov     [edi].Real_DS,_cwInit
        mov     [edi].Real_EDX,esi
        mov     [edi].Real_EAX,3c00h
        mov     [edi].Real_ECX,0
        mov     Real_SS[edi],0
        mov     Real_SP[edi],0
        mov     bl,21h
...
;
        ...
        mov     es,PSPSegment
        mov     ax,EnvSegment
        mov     es:[PSP_Environment],ax ;Setup ENV in PSP.
        mov     ax,RealENVSegment
        mov     es:w[EPSP_RealENV],ax
        mov     ax,offset Int21hExecCount
        mov     es:w[EPSP_ExecCount],ax
        mov     es:w[EPSP_ExecCount+2],ds
        mov     es:d[EPSP_Resource],edx ;Clear memory fields.
        mov     es:d[EPSP_INTMem],edx
        mov     es:d[EPSP_DPMIMem],edx
        mov     es:w[EPSP_Parent],es    ;set parent PSP.
        mov     es:w[EPSP_Next],dx
        ...
        mov     es:d[EPSP_mcbHead],edx
        mov     es:w[EPSP_SegBase],0
        mov     es:w[EPSP_SegSize],0
        mov     es:d[EPSP_LastPSP],0
        mov     es:d[EPSP_NextPSP],0
        mov     es:d[EPSP_Exports],0
        mov     es:d[EPSP_Imports],0
        mov     es:d[EPSP_Links],80000000h
        mov     es:d[EPSP_EntryCSEIP+4],0
...
@@d0:
        ...
        mov     Real_EAX[edi],900h
        mov     Real_DS[edi],_cwInit
